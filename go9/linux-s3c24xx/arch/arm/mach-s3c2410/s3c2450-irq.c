/* linux/arch/arm/mach-s3c2410/s3c2450-irq.c
 *
		      ...
		      struct pt_regs *regs)
{
	...
	if (subsrc & 1) {
		mydesc = irq_desc + IRQ_S3C2450_WDT;
		mydesc->handle(IRQ_S3C2450_WDT, mydesc, regs);
	}
	if (subsrc & 2) {
		mydesc = irq_desc + IRQ_S3C2450_AC97;
		mydesc->handle(IRQ_S3C2450_AC97, mydesc, regs);
	}
}
		  ...
		  struct pt_regs *regs)
{
	...
	if (subsrc & 1) {
		mydesc = irq_desc + IRQ_S3C2450_CAM_C;
		mydesc->handle(IRQ_S3C2450_CAM_C, mydesc, regs);
	}
	if (subsrc & 2) {
		mydesc = irq_desc + IRQ_S3C2450_CAM_P;
		mydesc->handle(IRQ_S3C2450_CAM_P, mydesc, regs);
	}
}
		  ...
		  struct pt_regs *regs)
{
	...
	if (subsrc & 1) {
		mydesc = irq_desc + IRQ_S3C2450_DMA0;
		mydesc->handle(IRQ_S3C2450_DMA0, mydesc, regs);
	}
	if (subsrc & 2) {
		mydesc = irq_desc + IRQ_S3C2450_DMA1;
		mydesc->handle(IRQ_S3C2450_DMA1, mydesc, regs);
	}
	if (subsrc & 4) {
		mydesc = irq_desc + IRQ_S3C2450_DMA2;
		mydesc->handle(IRQ_S3C2450_DMA2, mydesc, regs);
	}
	if (subsrc & 8) {
		mydesc = irq_desc + IRQ_S3C2450_DMA3;
		mydesc->handle(IRQ_S3C2450_DMA3, mydesc, regs);
	}
	if (subsrc & 16) {
		mydesc = irq_desc + IRQ_S3C2450_DMA4;
		mydesc->handle(IRQ_S3C2450_DMA4, mydesc, regs);
	}
	if (subsrc & 32) {
		mydesc = irq_desc + IRQ_S3C2450_DMA5;
		mydesc->handle(IRQ_S3C2450_DMA5, mydesc, regs);
	}
}
		  ...
		  struct pt_regs *regs)
{
	...
	if (subsrc & 2) {
		mydesc = irq_desc + IRQ_S3C2450_LCD2;
		mydesc->handle(IRQ_S3C2450_LCD2, mydesc, regs);
	}
	if (subsrc & 4) {
		mydesc = irq_desc + IRQ_S3C2450_LCD3;
		mydesc->handle(IRQ_S3C2450_LCD3, mydesc, regs);
	}
	if (subsrc & 8) {
		mydesc = irq_desc + IRQ_S3C2450_LCD4;
		mydesc->handle(IRQ_S3C2450_LCD4, mydesc, regs);
	}
}
		    ...
		    struct pt_regs *regs)
{
	...
	if (subsrc & 1) {
		mydesc = irq_desc + IRQ_S3C2450_RX3;
		mydesc->handle(IRQ_S3C2450_RX3, mydesc, regs);
	}
	if (subsrc & 2) {
		mydesc = irq_desc + IRQ_S3C2450_TX3;
		mydesc->handle(IRQ_S3C2450_TX3, mydesc, regs);
	}
	if (subsrc & 4) {
		mydesc = irq_desc + IRQ_S3C2450_ERR3;
		mydesc->handle(IRQ_S3C2450_ERR3, mydesc, regs);
	}
}
...
static int s3c2450_irq_add(struct sys_device *sysdev)
{
	unsigned int irqno;
	...
	for (irqno = IRQ_S3C2450_WDT; irqno <= IRQ_S3C2450_AC97; irqno++) {
	...
	for (irqno = IRQ_S3C2450_CAM_C; irqno <= IRQ_S3C2450_CAM_P; irqno++) {
	...
	for (irqno = IRQ_S3C2450_DMA0; irqno <= IRQ_S3C2450_DMA5; irqno++) {
	...
	for (irqno = IRQ_S3C2450_LCD2; irqno <= IRQ_S3C2450_LCD4; irqno++) {
	...
	for (irqno = IRQ_S3C2450_RX3; irqno <= IRQ_S3C2450_ERR3; irqno++) {
