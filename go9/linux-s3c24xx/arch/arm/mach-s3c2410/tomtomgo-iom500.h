#define SUPPORT_M500
inline void IO_InitM500(void)
{
	VALUE_SET(id                 , GOTYPE_M500);
	VALUE_SET(caseid             , GOCASE_MALAGA);
	VALUE_SET(cputype            , GOCPU_S3C2440);
	VALUE_SET(btchip             , GOBT_BC3);
	VALUE_SET(btspeed            , 921600);
	VALUE_SET(btclock            , 16000000);
	VALUE_SET(btclass            , 0x200408);
	VALUE_SET(handsfree          , 1);
	VALUE_SET(lowbutton          , 1);
	VALUE_SET(batterytype	     , GOBATTERY_ICR18650_2200);
	VALUE_SET(chargertype        , GOCHARGER_LTC1733);
	VALUE_SET(chargerresistor    , 1300);
	VALUE_SET(harddisk           , 1);
	VALUE_SET(gpio_autoshutdown  , 0);
	VALUE_SET(tfttype            , GOTFT_NEC_NL2432HC22);
	VALUE_SET(backlighttype	     , GOBACKLIGHT_CH0_TPS61042_350);
	VALUE_SET(tsxchannel         , 7);
	VALUE_SET(tsychannel         , 5);
	VALUE_SET(gpstype            , GOGPS_SIRF2);
	VALUE_SET(codectype          , GOCODEC_WM8971);
	VALUE_SET(acctype            , GOACC_MXR3999);
	VALUE_SET(usbslavetype       , GOUSB_TUSB6250);
	VALUE_SET(pnp                , 1);
	VALUE_SET(usbdevicehostcapable,0);
	VALUE_SET(ohciports          , 0x00);
	VALUE_SET(harddisktiming     , 4);
	VALUE_SET(canrecordaudio     , 1);

	STRING_SET(name,"TomTom GO 700");
	STRING_SET(btname,"TomTom GO 700");
	STRING_SET(usbname,"GO 700");
	STRING_SET(familyname,"TomTom GO");
	STRING_SET(projectname,"MALAGA");
	STRING_SET(requiredbootloader,"2.18");
	STRING_SET(btdev,"ttySAC1");
	STRING_SET(gpsdev,"ttySAC2");

	PIN_SET(GPS_RESET       , PIN_GPC5 | PIN_INVERTED);
	PIN_SET(GPS_ON          , PIN_GPJ6);
	PIN_SET(GPS_1PPS        , PIN_GPG0);
	PIN_SET(GPS_REPRO       , PIN_GPG1);
	PIN_SET(HDD_LED         , PIN_GPD9 | PIN_INVERTED);
	PIN_SET(HDD_PWR_ON      , PIN_GPD8 | PIN_INVERTED);
	PIN_SET(HDD_RST         , PIN_GPC8 | PIN_INVERTED);
	PIN_SET(HDD_BUF_EN      , PIN_GPD1 | PIN_INVERTED);
	PIN_SET(HDD_IRQ         , PIN_GPG11);
	PIN_SET(HDD_DRQ         , PIN_GPB9);
	PIN_SET(HDD_DACK        , PIN_GPB10);
	PIN_SET(HDD_CS          , PIN_GPA14 | PIN_INVERTED);
	PIN_SET(CHARGEFAULT     , PIN_GPJ1 | PIN_INVERTED);
	PIN_SET(CHARGE_OUT      , PIN_GPJ7 | PIN_OPEN_EMITTER);
	PIN_SET(CHARGING        , PIN_GPJ12 | PIN_INVERTED);
	PIN_SET(AIN4_PWR        , PIN_GPA18);
	PIN_SET(LIGHTS_DETECT   , PIN_GPF3 | PIN_INVERTED);
	PIN_SET(DOCK_PWREN      , PIN_GPJ9 | PIN_INVERTED);
	PIN_SET(DOCK_SENSE      , PIN_GPJ10 | PIN_INVERTED);
	PIN_SET(FSK_FFS         , PIN_GPJ5);
	PIN_SET(FSK_FFE         , PIN_GPF5);
	PIN_SET(FSK_IRQ         , PIN_GPG8 | PIN_INVERTED);
	PIN_SET(SPICLK          , PIN_GPE13);
	PIN_SET(SPIMSI          , PIN_GPE12);
	PIN_SET(SPIMSO          , PIN_GPE11);
	PIN_SET(FSK_EN          , PIN_GPJ2);
	PIN_SET(FSK_CLKOUT      , PIN_GPH10);
	PIN_SET(ACC_PWR_ON      , PIN_GPJ4 | PIN_INVERTED);
	PIN_SET(AMP_ON          , PIN_GPJ3);
	PIN_SET(DAC_PWR_ON      , PIN_GPA22);
	PIN_SET(MUTE_EXT        , PIN_GPA19);
	PIN_SET(MUTE_INT        , PIN_GPF6);
	PIN_SET(I2SSDO          , PIN_GPE4);
	PIN_SET(CDCLK           , PIN_GPE2);
	PIN_SET(I2SSCLK         , PIN_GPE1);
	PIN_SET(I2SLRCK         , PIN_GPE0);
	PIN_SET(I2SSDI          , PIN_GPE3);
	PIN_SET(L3CLOCK         , PIN_GPB4);
	PIN_SET(L3MODE          , PIN_GPB3);
	PIN_SET(L3DATA          , PIN_GPB2);
	PIN_SET(CTS_DOCK        , PIN_GPH0);
	PIN_SET(RTS_DOCK        , PIN_GPH1);
	PIN_SET(TXD_DOCK        , PIN_GPH2);
	PIN_SET(RXD_DOCK        , PIN_GPH3);
	PIN_SET(TXD_GPS         , PIN_GPH6);
	PIN_SET(RXD_GPS         , PIN_GPH7);
	PIN_SET(TXD_BT          , PIN_GPH4);
	PIN_SET(RXD_BT          , PIN_GPH5);
	PIN_SET(RTS_BT          , PIN_GPG9);
	PIN_SET(CTS_BT          , PIN_GPG10);
	PIN_SET(LCD_VCC_PWREN   , PIN_GPG4 | PIN_INVERTED);
	PIN_SET(LCD_BIAS_PWREN  , PIN_GPC7);
	PIN_SET(LCD_OEN         , PIN_GPC6);
	PIN_SET(BACKLIGHT_EN    , PIN_GPB1 | PIN_INVERTED);
	PIN_SET(BACKLIGHT_PWM   , PIN_GPB0);
	PIN_SET(USB_HOST_DETECT , PIN_GPF1 | PIN_INVERTED);
	PIN_SET(USB_PWR_EN      , PIN_GPD0 | PIN_INVERTED);
	PIN_SET(USB_SUSPEND     , PIN_GPJ0);
	PIN_SET(USB_EJECT       , PIN_GPH8);
	PIN_SET(USB_CLKOUT      , PIN_GPH9);
	PIN_SET(USB_RST         , PIN_GPC9 | PIN_INVERTED);
	PIN_SET(HW_IIC_SDA      , PIN_GPE15);
	PIN_SET(HW_IIC_SCL      , PIN_GPE14);
	PIN_SET(ON_OFF          , PIN_GPF0 | PIN_INVERTED);
	PIN_SET(IGNITION        , PIN_GPF7 | PIN_INVERTED);
	PIN_SET(ACPWR           , PIN_GPJ11 | PIN_INVERTED);
	PIN_SET(XMON            , PIN_GPG15 | PIN_OPEN_COLLECTOR);
	PIN_SET(XPON            , PIN_GPG14 | PIN_OPEN_EMITTER);
	PIN_SET(YMON            , PIN_GPG13 | PIN_OPEN_COLLECTOR);
	PIN_SET(YPON            , PIN_GPG12 | PIN_OPEN_EMITTER);
	PIN_SET(TSDOWN          , PIN_GPG15 | PIN_OPEN_COLLECTOR);
	PIN_SET(FACTORY_TEST_POINT , PIN_GPC0);
}
