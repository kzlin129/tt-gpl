#define SUPPORT_AUSTIN
static backlight_mapping_t AustinMapTable = {
	17, 23, 29, 34, 38, 42, 44, 47, 50, 53, 56, 59, 63, 66, 69, 72, 76, 80, 84, 87, 90 };

inline void IO_InitAustin(void)
{
	VALUE_SET(id                 , GOTYPE_AUSTIN);
	...
	VALUE_SET(backlight_mapping  , &AustinMapTable);
	VALUE_SET(backlightfreq      ,20000);//New, backlight frequence = 20kHz

	STRING_SET(name,"TomTom XXL IQ Routes Edition");
	STRING_SET(usbname,"ONE XXL IQ Routes");
	STRING_SET(familyname,"TomTom ONE");
	STRING_SET(projectname,"Austin");
