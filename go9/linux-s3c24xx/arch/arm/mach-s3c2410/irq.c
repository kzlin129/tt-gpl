...
s3c_irqext_bit(unsigned int irqno)
{
	unsigned long bit;

	if (irqno >= IRQ_EINT0 && irqno <= IRQ_EINT3) {
		bit = 1UL << (irqno - IRQ_EINT0);
	} else if (irqno >= IRQ_EINT4 && irqno <= IRQ_EINT23) {
		bit = 1UL << (irqno - EXTINT_OFF);
	} else {
		panic("Impossible external IRQ %d\n", irqno);
	}
	/* assertion:
	 * (irqno >= IRQ_EINT0 && irqno <= IRQ_EINT3) ||
	 * (irqno >= IRQ_EINT4 && irqno <= IRQ_EINT23
	 */

	return bit;
}

#define EINT4t7_MASK  (0xfUL << 4)
#define EINT8t23_MASK (0xffffUL << 8)
...
s3c_irqext_mask(unsigned int irqno)
{
	...
	if (irqno <= IRQ_EINT3) {
		/* also mask parent irq */
		s3c_irq_mask(irqno);
	} else if (irqno <= IRQ_EINT7) {
		/* check to see if EINT4 through EINT7 need masking */

		if ((mask & EINT4t7_MASK) == EINT4t7_MASK) {
			/* all masked, mask the parent too */
			s3c_irq_mask(IRQ_EINT4t7);
		}
	} else {
		/* check to see if EINT8 through EINT23 need masking */

		if ((mask & EINT8t23_MASK) == EINT8t23_MASK) {
			/* all masked, mask the parent too */
...
s3c_irqext_ack(unsigned int irqno)
{
	...
	if (irqno <= IRQ_EINT3) {
		...
		if ((req & EINT4t7_MASK) == 0) {
			s3c_irq_ack(IRQ_EINT4t7);
		}
	} else {
		if ((req & EINT8t23_MASK) == 0) {
...
s3c_irqext_type(unsigned int irq, unsigned int type)
{
	...
	unsigned long extint_val, gpcon_offset, extint_offset;
	...
	if ((irq >= IRQ_EINT0) && (irq <= IRQ_EINT3))
	{
		...
		extint_val = s3c24xx_read_extint0();
	...
	else if ((irq >= IRQ_EINT8) && (irq <= IRQ_EINT15))
	{
		...
		extint_val = s3c24xx_read_extint1();
	...
	else if ((irq >= IRQ_EINT16) && (irq <= IRQ_EINT23))
	{
		...
		extint_val = s3c24xx_read_extint2();
		gpcon_offset = (irq - IRQ_EINT8) * 2;
