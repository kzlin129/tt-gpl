#define SUPPORT_FLORENCE
static backlight_mapping_t FlorenceMapTable = {17,22,26,31,34,36,38,40,42,45,47,
	                49,52,54,57,59,62,64,67,70,72};
inline void IO_InitFlorence(void)
{
	VALUE_SET(id                 , GOTYPE_FLORENCE);//New for Florence
	...
	VALUE_SET(backlight_mapping  , &FlorenceMapTable);
	
	STRING_SET(name,"TomTom XL LIVE IQ Routes Edition");
	STRING_SET(usbname,"XL LIVE IQ Routes Edition");
	STRING_SET(familyname,"TomTom ONE");
	STRING_SET(projectname,"Florence");
	STRING_SET(requiredbootloader,"5.4218");
	STRING_SET(gprsmodemdev,"ttySAC0");//Modem use UART0
	STRING_SET(gpsdev,"ttySAC1");//GPS use UART1
	STRING_SET(dockdev,"ttySAC2");//JTAG use UART2
...
// B Group
	...
	PIN_SET(GSM_SYS_EN         , PIN_GPB9);//New for Florence
	PIN_SET(GSM_SYS_RST        , PIN_GPB10);//New for Florence
// C Group
...
// F Group
	...
	//PIN_SET(GPS_POWERON_OUT    , PIN_GPF3);//New for Florence
	PIN_SET(GPS_STANDBY        , PIN_GPF4| PIN_INVERTED);//New for Florence
...
// H Group
	PIN_SET(GSM_CTS            , PIN_GPH0);//New for Florence
	PIN_SET(GSM_RTS            , PIN_GPH1);//New for Florence
	PIN_SET(GSM_TXD            , PIN_GPH2);//New for Florence
	PIN_SET(GSM_RXD            , PIN_GPH3);//New for Florence
	PIN_SET(TXD_GPS            , PIN_GPH4);
	PIN_SET(RXD_GPS            , PIN_GPH5);
	PIN_SET(TXD_DOCK           , PIN_GPH6);//New for Florence
	PIN_SET(RXD_DOCK           , PIN_GPH7);//New for Florence
	PIN_SET(MOVI_PWR_ON        , PIN_GPH8 | PIN_INVERTED);//New for Florence
	PIN_SET(CDCLK_12MHZ        , PIN_GPH10); 	

}

inline void IO_InitFlorenceFlorin( void )
{
	STRING_SET(name,"TomTom XL 340S LIVE"); /* important: leave " \0" after GO for IO_GetModelVariant() checking in blit.c */
	STRING_SET(usbname,"XL 340S LIVE");
}
