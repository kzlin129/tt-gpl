...
EXPORT_SYMBOL(reset_state);

char boot_device_id[24];
EXPORT_SYMBOL(boot_device_id);

unsigned int atag_tfttype;
EXPORT_SYMBOL(atag_tfttype);

unsigned int atag_feature[2];
EXPORT_SYMBOL(atag_feature);
...
static void dump_cpu_info(int cpu)
{
...
#if defined(CONFIG_VGA_CONSOLE) || defined(CONFIG_DUMMY_CONSOLE) || defined(CONFIG_MACH_TOMTOMGO)
...
static int __init parse_tag_boot_device(const struct tag *tag)
{
	/* Copy device ID of our boot device _safely_ */
	memcpy(boot_device_id, tag->u.boot_device.device_id, sizeof(boot_device_id));
	boot_device_id[sizeof(boot_device_id)-1] = '\0';
	
	return 0;
}

__tagtable(ATAG_BOOT_DEVICE, parse_tag_boot_device);


static int __init parse_tag_tft_type(const struct tag *tag)
{
	atag_tfttype = tag->u.tft_type.type;
	return 0;
}

__tagtable(ATAG_TFT_TYPE, parse_tag_tft_type);

static int __init parse_tag_feature(const struct tag *tag)
{
	memcpy(atag_feature, tag->u.feature.type, sizeof(atag_feature));
	return 0;
}

__tagtable(ATAG_FEATURE, parse_tag_feature);

static int __init parse_tag_reset_state(const struct tag *tag)
{
	/* set when bootloader has detected a cold boot */
	reset_state = tag->u.reset_state.state;
	return 0;
}

__tagtable(ATAG_RESET_STATE, parse_tag_reset_state);
