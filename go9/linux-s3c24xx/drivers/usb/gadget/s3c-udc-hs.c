 ...
 *
 * $Id: s3c-udc-hs.c,v 1.2 2007/02/16 11:23:02 ita Exp $*
 *
...
#define DEBUG_S3C_HSUDC_SETUP
#define DEBUG_S3C_HSUDC_EP0
#define DEBUG_S3C_UDC

#ifdef DEBUG_S3C_HSUDC_SETUP
...
static const char driver_name[] = "s3c-hsudc";
...
{
	__raw_writel(ind, S3C_HSUDC_INDEX_REG);
				   ...
				   struct s3c_request *req, int max)
{
	...
	usb_write(length, (u32) S3C_HSUDC_BYTE_WRITE_CNT_REG, ep_index(ep));

        
        if(length == EP_FIFO_SIZE) {
		count = length;
...
static int read_fifo(struct s3c_ep *ep, struct s3c_request *req)
{
	u32 csr;
	u16 *buf;
	unsigned bufferspace, count, count_bytes, is_short;
	u32 fifo = ep->fifo;

	csr = usb_read( (u32) S3C_HSUDC_EP_STATUS_REG, ep_index(ep));

	/* make sure there's a packet in the FIFO. */
	if (!(csr & S3C_HSUDC_EP_RX_SUCCESS)) {
	...
	count = usb_read((u32) S3C_HSUDC_BYTE_READ_CNT_REG, ep_index(ep));
	if (csr & S3C_HSUDC_EP_LWO)
...
static void s3c_in_epn(struct s3c_udc *dev, u32 ep_idx)
{
	...
	csr = usb_read((u32) S3C_HSUDC_EP_STATUS_REG, ep_idx);
	DEBUG("%s: S3C_HSUDC_EP%d_STATUS_REG=0x%x\n", __FUNCTION__, ep_idx, csr);

	if (csr & S3C_HSUDC_EP_STALL) {
		DEBUG("%s: S3C_HSUDC_EP_STALL\n", __FUNCTION__);
		usb_set(S3C_HSUDC_EP_STALL, (u32) S3C_HSUDC_EP_STATUS_REG, ep_idx);
		return;
	}
	...
	if (!req) {
		if (csr & S3C_HSUDC_EP_TX_SUCCESS) {
			DEBUG("%s: EP_TX_SUCCESS, req = %p\n", __FUNCTION__, req);
			usb_set(S3C_HSUDC_EP_TX_SUCCESS,
				(u32) S3C_HSUDC_EP_STATUS_REG, ep_idx);
		}
		return;
	}
	...
	if (csr & S3C_HSUDC_EP_TX_SUCCESS) {
		...
		usb_set(S3C_HSUDC_EP_TX_SUCCESS,
			(u32) S3C_HSUDC_EP_STATUS_REG, ep_idx);
		if ((write_fifo(ep, req)==0)
			    && (csr & S3C_HSUDC_EP_PSIF_TWO))
				write_fifo(ep, req);
	}
}
...
static void s3c_out_epn(struct s3c_udc *dev, u32 ep_idx)
{
	...
	if (csr & S3C_HSUDC_EP_FIFO_FLUSH) {
		DEBUG("%s: fifo flush \n", __FUNCTION__);
		usb_set(S3C_HSUDC_EP_FIFO_FLUSH, (u32) S3C_HSUDC_EP_CON_REG, ep_idx);
		return;
	}
	...
	if (csr & S3C_HSUDC_EP_RX_SUCCESS) {
		...
		if (unlikely(!req)) {
			...
			if(((read_fifo(ep, req))==0) && (csr & S3C_HSUDC_EP_PSIF_TWO))
				read_fifo(ep, req);
		}
	}
}
...
static void reconfig_usbd(void)
{
	__raw_writel(0x04, S3C_HSUDC_EP_DIR_REG); /* EP1:OUT, EP2:IN */

	/* EP0~2 Interrupt enable */	
	__raw_writel(0x7, S3C_HSUDC_EP_INT_EN_REG);
	 ...
	 */
	__raw_writel(S3C_HSUDC_DTZIEN_EN|S3C_HSUDC_RRD_EN
			|S3C_HSUDC_SUS_EN|S3C_HSUDC_RST_EN,
			S3C_HSUDC_SYS_CON_REG);
	
	__raw_writel(0x0000, S3C_HSUDC_EP0_CON_REG);

	/* EP1, EP2 dual FIFO mode enable */
	usb_write(0x0080, (u32) S3C_HSUDC_EP_CON_REG, 1);
	usb_write(0x0080, (u32) S3C_HSUDC_EP_CON_REG, 2);

	__raw_writel(0, S3C_HSUDC_INDEX_REG);
...
void set_max_pktsize(struct s3c_udc *dev, enum usb_device_speed speed)
{
	...
	usb_write(ep0_fifo_size, (u32) S3C_HSUDC_MAXP_REG, 0);
	usb_write(ep_fifo_size, (u32) S3C_HSUDC_MAXP_REG, 1);
	usb_write(ep_fifo_size, (u32) S3C_HSUDC_MAXP_REG, 2);
	usb_write(ep_fifo_size, (u32) S3C_HSUDC_MAXP_REG, 3);
	usb_write(ep_fifo_size, (u32) S3C_HSUDC_MAXP_REG, 4);
	usb_write(ep_fifo_size2, (u32) S3C_HSUDC_MAXP_REG, 5);
	usb_write(ep_fifo_size2, (u32) S3C_HSUDC_MAXP_REG, 6);
	usb_write(ep_fifo_size2, (u32) S3C_HSUDC_MAXP_REG, 7);
	usb_write(ep_fifo_size2, (u32) S3C_HSUDC_MAXP_REG, 8);
...
static irqreturn_t s3c_udc_irq(int irq, void *_dev, struct pt_regs *r)
{
	...
	intr_status = __raw_readl(S3C_HSUDC_SYS_STATUS_REG);
	intr_out = intr_in = __raw_readl(S3C_HSUDC_EP_INT_REG);
	
	DEBUG_SETUP("\n\n%s: S3C_HSUDC_EP_INT_REG=0x%x, S3C_HSUDC_SYS_STATUS_REG=0x%x(on state %s)\n",
			__FUNCTION__, intr_out, intr_status, state_names[dev->ep0state]);

	/* We have only 3 usable eps now 	-jassi */
	intr_status_chk = intr_status & S3C_HSUDC_INT_CHECK;
	intr_in &= (S3C_HSUDC_INT_EP2 | S3C_HSUDC_INT_EP0);
	intr_out &= S3C_HSUDC_INT_EP1;
	...
	if (intr_status) {
		if (intr_status & S3C_HSUDC_INT_VBUSON) {
			DEBUG_SETUP("%s: VBUSON interrupt\n", __FUNCTION__);
			__raw_writel(S3C_HSUDC_INT_VBUSON, S3C_HSUDC_SYS_STATUS_REG);
		}

		if (intr_status & S3C_HSUDC_INT_ERR) {
			DEBUG_SETUP("%s: Error interrupt\n", __FUNCTION__);
			__raw_writel(S3C_HSUDC_INT_ERR, S3C_HSUDC_SYS_STATUS_REG);
		}

		if (intr_status & S3C_HSUDC_INT_SDE) {
			DEBUG_SETUP("%s: Speed Detection interrupt\n",
					__FUNCTION__);
			__raw_writel(S3C_HSUDC_INT_SDE, S3C_HSUDC_SYS_STATUS_REG);

			if (intr_status & S3C_HSUDC_INT_HSP) {
		...
		if (intr_status & S3C_HSUDC_INT_SUSPEND) {
			DEBUG_SETUP("%s: SUSPEND interrupt\n", __FUNCTION__);
			__raw_writel(S3C_HSUDC_INT_SUSPEND, S3C_HSUDC_SYS_STATUS_REG);
			if (dev->gadget.speed != USB_SPEED_UNKNOWN
			    && dev->driver
			    && dev->driver->suspend) {
		...
		if (intr_status & S3C_HSUDC_INT_RESUME) {
			DEBUG_SETUP("%s: RESUME interrupt\n", __FUNCTION__);
			__raw_writel(S3C_HSUDC_INT_RESUME, S3C_HSUDC_SYS_STATUS_REG);
			if (dev->gadget.speed != USB_SPEED_UNKNOWN
			    && dev->driver
			    && dev->driver->resume) {
		...
		if (intr_status & S3C_HSUDC_INT_RESET) {
			DEBUG_SETUP("%s: RESET interrupt\n", __FUNCTION__);
			__raw_writel(S3C_HSUDC_INT_RESET, S3C_HSUDC_SYS_STATUS_REG);
			reconfig_usbd();
			dev->ep0state = WAIT_FOR_SETUP;
		}
	}
	...
	if (intr_in) {
		if (intr_in & S3C_HSUDC_INT_EP0){
			__raw_writel(S3C_HSUDC_INT_EP0, S3C_HSUDC_EP_INT_REG);
			s3c_handle_ep0(dev);
		}else if (intr_in & S3C_HSUDC_INT_EP2){
			__raw_writel(S3C_HSUDC_INT_EP2, S3C_HSUDC_EP_INT_REG);
			s3c_in_epn(dev, 2);	// hard coded !!!   -jassi
		}else{
			__raw_writel(S3C_HSUDC_INT_EP3, S3C_HSUDC_EP_INT_REG);
			s3c_in_epn(dev, 3);	// hard coded !!!   -jassi
		}
	}

	if (intr_out) {
		usb_write(S3C_HSUDC_INT_EP1, (u32) S3C_HSUDC_EP_INT_REG, 1);
			 ...
			 gfp_t gfp_flags)
{
	...
	if (list_empty(&ep->queue) && likely(!ep->stopped)) {
		...
		if (unlikely(ep_index(ep) == 0)) {
			...
			csr = usb_read((u32) S3C_HSUDC_EP_STATUS_REG, ep_index(ep));
			DEBUG("%s: ep_is_in, S3C_HSUDC_EP%d_STATUS_REG=0x%x\n",
				__FUNCTION__, ep_index(ep), csr);
			...
			csr = usb_read((u32) S3C_HSUDC_EP_STATUS_REG, ep_index(ep));
			DEBUG("%s: ep_is_out, S3C_HSUDC_EP%d_STATUS_REG=0x%x\n",
				__FUNCTION__, ep_index(ep),csr);
...
static int s3c_fifo_status(struct usb_ep *_ep)
{
	...
	if (ep->dev->gadget.speed != USB_SPEED_UNKNOWN ||
	    csr & S3C_HSUDC_EP_RX_SUCCESS) {
	    
		count = usb_read((u32) S3C_HSUDC_BYTE_READ_CNT_REG, ep_index(ep));
		
		if (usb_read((u32) S3C_HSUDC_EP_STATUS_REG, ep_index(ep))
			& S3C_HSUDC_EP_LWO)
			count = count * 2 -1;
		else
			count = count * 2;
...
static __inline__ int s3c_fifo_read(struct s3c_ep *ep, u16 *cp, int max)
{
	...
	while (count--) {
		*cp++ = (u16) __raw_readl(S3C_HSUDC_EP0_FIFO_REG);
	}

	__raw_writel(S3C_HSUDC_EP0_RX_SUCCESS, S3C_HSUDC_EP0_STATUS_REG);/* clear */
...
static int read_fifo_ep0(struct s3c_ep *ep, struct s3c_request *req)
{
	...
	csr = __raw_readl(S3C_HSUDC_EP0_STATUS_REG);
	if (!(csr & S3C_HSUDC_EP0_RX_SUCCESS))
		return 0;
	...
	/* read all bytes from this packet */
	if (likely(csr & S3C_HSUDC_EP0_RX_SUCCESS)) {
		count = usb_read((u32) S3C_HSUDC_BYTE_READ_CNT_REG, ep_index(ep));
		if (csr & S3C_HSUDC_EP0_LWO)
			bytes = count * 2 -1;
		else
			bytes = count * 2;
...
static void udc_set_address(struct s3c_udc *dev, unsigned char address)
{
	DEBUG_EP0("%s: address=%d, S3C_HSUDC_FUNC_ADDR_REG=0x%x\n",
		__FUNCTION__, address, __raw_readl(S3C_HSUDC_FUNC_ADDR_REG));
...
static void s3c_ep0_read(struct s3c_udc *dev)
{
	...
	if(req->req.length == 0) {
		__raw_writel(S3C_HSUDC_EP0_RX_SUCCESS, S3C_HSUDC_EP0_STATUS_REG);
		dev->ep0state = WAIT_FOR_SETUP;
		first_time = 1;
		done(ep, req, 0);
		return;
...
static void s3c_handle_ep0(struct s3c_udc *dev)
{
	...
	DEBUG_EP0("%s: S3C_HSUDC_EP0_STATUS_REG = 0x%x\n", __FUNCTION__, csr);

	/*
	 * if SENT_STALL is set
	 *      - clear the SENT_STALL bit
	 */
	if (csr & S3C_HSUDC_EP0_STALL) {
		printk("%s: S3C_HSUDC_EP0_STALL\n", __FUNCTION__);
		__raw_writel(S3C_HSUDC_EP0_STALL, S3C_HSUDC_EP0_STATUS_REG); /* clear */
		nuke(ep, -ECONNABORTED);
		dev->ep0state = WAIT_FOR_SETUP;
		return;
	}
		
	if (csr & S3C_HSUDC_EP0_TX_SUCCESS) {
		DEBUG_EP0("%s: EP0_TX_SUCCESS \n", __FUNCTION__);
		__raw_writel(S3C_HSUDC_EP0_TX_SUCCESS, S3C_HSUDC_EP0_STATUS_REG); /* clear */
	}
...
static int s3c_udc_get_frame(struct usb_gadget *_gadget)
{
	...
	u32 frame = __raw_readl(S3C_HSUDC_FRAME_NUM_REG);
...
static struct s3c_udc memory = {
	...
	.ep[0] = {
		  ...
		  .ep_type = ep_control,
		  .fifo = (u32) S3C_HSUDC_EP0_FIFO_REG,
		  },
	...
	.ep[1] = {
		  ...
		  .ep_type = ep_bulk_out,
		  .fifo = (u32) S3C_HSUDC_EP1_FIFO_REG,
		  },
	...
	.ep[2] = {
		  ...
		  .ep_type = ep_bulk_in,
		  .fifo = (u32) S3C_HSUDC_EP2_FIFO_REG,
		  },
	...
	.ep[3] = {				// Though NOT USED XXX
		  ...
		  .fifo = (u32) S3C_HSUDC_EP3_FIFO_REG,
		  },
	.ep[4] = {				// Though NOT USED XXX
		  ...
		  .ep_type = ep_interrupt,
		  .fifo = (u32) S3C_HSUDC_EP4_FIFO_REG,
		  },
	.ep[5] = {				// Though NOT USED XXX
		  ...
		  .ep_type = ep_interrupt,
		  .fifo = (u32) S3C_HSUDC_EP5_FIFO_REG,
		  },
	.ep[6] = {				// Though NOT USED XXX
		  ...
		  .ep_type = ep_interrupt,
		  .fifo = (u32) S3C_HSUDC_EP6_FIFO_REG,
		  },
	.ep[7] = {				// Though NOT USED XXX
		  ...
		  .ep_type = ep_interrupt,
		  .fifo = (u32) S3C_HSUDC_EP7_FIFO_REG,
		  },
	.ep[8] = {				// Though NOT USED XXX
		  ...
		  .ep_type = ep_interrupt,
		  .fifo = (u32) S3C_HSUDC_EP8_FIFO_REG,
		  },
};
 ...
 */
static int s3c_udc_probe(struct device* _dev)
{
...
static int s3c_udc_remove(struct device* _dev)
{
	struct platform_device* pdev = to_platform_device(_dev);
...
/*-------------------------------------------------------------------------*/
static struct device_driver s3c_udc_driver = {
	.owner		= THIS_MODULE,
	.name		= "s3c-hsudc",
...
static int __init udc_init(void)
{
	...
	ret = driver_register(&s3c_udc_driver);
	if(!ret)
...
{
	driver_unregister(&s3c_udc_driver);
