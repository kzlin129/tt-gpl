...
#define spin_lock_irqsaveARM(lock, flags) do { (void)(flags); spin_lock(lock); } while (0)
#define spin_unlock_irqrestoreARM(lock, flags) do { (void)(flags); spin_unlock(lock); } while (0)
#define spin_lock_irqARM(lock) spin_lock(lock)
#define spin_unlock_irqARM(lock) spin_unlock(lock)
...
int ide_end_request (ide_drive_t *drive, int uptodate, int nr_sectors)
{
	...
	spin_lock_irqsaveARM(&ide_lock, flags);
	rq = HWGROUP(drive)->rq;
	...
	spin_unlock_irqrestoreARM(&ide_lock, flags);
	return ret;
}
...
u64 ide_get_error_location(ide_drive_t *drive, char *args)
{
	...
	if (ide_id_has_flush_cache_ext(drive->id) && IDE_CONTROL_REG) {
...
static ide_startstop_t start_request (ide_drive_t *drive, struct request *rq)
{
	...
	if (blk_pm_suspend_request(rq) &&
		...
		SELECT_DRIVE(drive);
		if(HWIF(drive)->io_ports[IDE_CONTROL_OFFSET])
...
static void ide_irq_tasklet(unsigned long data);
DECLARE_TASKLET(ide_irq_tasklet_handle, ide_irq_tasklet, 0L);

static int irq;
static void* dev_id;
static struct pt_regs *regs; 

irqreturn_t ide_intr (int irq2, void *dev_id2, struct pt_regs *regs2)
{
	unsigned long flags;
	spin_lock_irqsave(&ide_lock, flags);
	irq = irq2;
	...
	regs = regs2;
	disable_irq(irq);
	tasklet_schedule(&ide_irq_tasklet_handle);
	spin_unlock_irqrestore(&ide_lock, flags);
	return IRQ_HANDLED;
}
