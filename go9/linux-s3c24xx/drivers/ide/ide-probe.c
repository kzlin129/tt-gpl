...
static int do_probe (ide_drive_t *drive, u8 cmd)
{
	...
	if ((hwif->INB(IDE_SELECT_REG) & (1<<4)) != (drive->select.b.unit<<4) && !drive->present) {
	...
	// When probe succeeded set idle timeout to 5 seconds on Valencia
	if (0)
	if (rc == 0)
	{
		unsigned long timeout;
		timeout = jiffies;
		hwif->OUTB(0xff, IDE_NSECTOR_REG);
		hwif->OUTB(WIN_SETIDLE1, IDE_COMMAND_REG);
		while (((hwif->INB(IDE_STATUS_REG)) & BUSY_STAT) &&
		       time_before(jiffies, timeout + WAIT_WORSTCASE));
	}
	if (drive->select.b.unit != 0) {
