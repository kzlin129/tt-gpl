...
static struct usb_device_id blacklist_ids[] = {
	...
	/* CSR BlueCore ROM variants needs PS_KEY settings before going into operational HCI mode */
	/* ignore and initialise via third-party program and libusb, then reboot with different productid */
	{ USB_DEVICE(0x0a12, 0x0001), .driver_info = HCI_IGNORE },
...
static int hci_usb_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (reset || id->driver_info & HCI_CSR_BCROM) {
