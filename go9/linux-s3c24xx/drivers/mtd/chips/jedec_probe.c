...
#define EON29SL400BB	0x22F1 
#define EON29SL800BB	0x226B
#define EON29SL160BB	0x22E7
#define EON39SL800		0x273F
...
{
#if 1
	// Replacing back old workaround for bypassing detection,
	// use only EON29LV400BB. Too many EON chips are broken.
	int i;
	u32 mfr = 0;
	u32 id = 0;
	int chip_id = EON29LV400BB; /* default */

	/* MTD_UADDR_0x0555_0x02AA */
	cfi->addr_unlock1 = unlock_addrs[1].addr1;
	cfi->addr_unlock2 = unlock_addrs[1].addr2;
	/* Reset */
	jedec_reset(base, map, cfi);
	/* Autoselect Mode */
	cfi_send_gen_cmd(0xaa, cfi->addr_unlock1, base, map, cfi, cfi->device_type, NULL);
	...
	mfr = jedec_read_mfr(map, base, cfi);
	id = jedec_read_id(map, base, cfi);
	printk(KERN_WARNING "MTD: %s(): read: man=0x%x id=0x%x\n",
				__func__, mfr, id);
	if ( mfr == MANUFACTURER_EON )
	{
		switch ( id ) {
			case EON39SL800:
			case EON29SL160BB:
				printk(KERN_WARNING "MTD: %s(): detected EON %x\n",
					__func__, id);
				chip_id = id;
				break;
			default:
	...
	{
		if ((jedec_table[i].mfr_id == MANUFACTURER_EON) &&
				(jedec_table[i].dev_id == chip_id))
		{
			if (!cfi_jedec_setup(cfi, i))
				return 0;
			else
			{
				jedec_reset(base, map, cfi); /* back to read mode */
				return 1;
			}
		}
	}
