...
/* Include some tomtom GO specific extensions	*/
#include "tomtom-uart.h"
...
static void serial8250_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
	...
	if (mctrl & TIOCM_OUT1)
		mcr |= UART_MCR_OUT1;
#if defined CONFIG_BARCELONA_EXTERNAL_UART || defined CONFIG_BARCELONA_EXTERNAL_UART_MODULE
	/* On murcia some genius pull-upped the IRQ line 	*/
	/* so we need to keep driving the IRQ line and never tristate it	*/
	mcr |= UART_MCR_OUT2;	
#else
	if (mctrl & TIOCM_OUT2)
		mcr |= UART_MCR_OUT2;
...
static int serial8250_startup(struct uart_port *port)
{
	 ...
	 */
	if ((up->port.type == PORT_16850) || (up->port.type == PORT_16570) ){
		unsigned char fctr;
	...
	/* NOTE: I changed the order of initialization of the UART and the
	 * registration of the ISR. On TomTom boards the interrupt is level-converted
	 * and if the TIOCM_OUT2 in the MCR is not set, the output on an EXAR 16L570 is tristated
	 * and the output of the level converter triggers IRQ's in the cpu,
...
#if defined CONFIG_BARCELONA_EXTERNAL_UART || defined CONFIG_BARCELONA_EXTERNAL_UART_MODULE
		hw_tomtom_external_uart_suspend(); // Put external uart in power save mode.
#endif //CONFIG_BARCELONA_EXTERNAL_UART
...
#if defined CONFIG_BARCELONA_EXTERNAL_UART || defined CONFIG_BARCELONA_EXTERNAL_UART_MODULE
		hw_tomtom_external_uart_resume(); // Get external uart back from power save mode.
#endif //CONFIG_BARCELONA_EXTERNAL_UART
...
#if defined CONFIG_BARCELONA_EXTERNAL_UART || defined CONFIG_BARCELONA_EXTERNAL_UART_MODULE
	if( port->uartclk == 0xffffffff )
	{
		port->uartclk = hw_tomtom_external_uart_get_clock_rate ( );
		// might return 0 if there is an error with the clock!
	}
#endif //CONFIG_BARCELONA_EXTERNAL_UART
...
#if defined CONFIG_BARCELONA_EXTERNAL_UART || defined CONFIG_BARCELONA_EXTERNAL_UART_MODULE
		hw_tomtom_external_uart_init(); // do it before we might end up probing ...
#endif //CONFIG_BARCELONA_EXTERNAL_UART
