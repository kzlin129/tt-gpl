...
static inline int s3c24xx_i2c_is2440(struct s3c24xx_i2c *i2c)
{
	...
	return strcmp(pdev->name, "s3c2410-i2c") != 0;
}
...
static int s3c24xx_i2c_init(struct s3c24xx_i2c *i2c)
{
	...
	if (s3c24xx_i2c_getdivisor(i2c, pdata, &iicon, &freq, clkin) != 0) {
...
#if defined CONFIG_CPU_FREQ && defined CONFIG_S3C24XX_DFS_CPUFREQ
static int i2c_freq_transition(struct notifier_block *nb, unsigned long val, void *data)
{
	struct cpufreq_freqs		*f = data;
	struct s3c24xx_i2c		*i2c=(struct s3c24xx_i2c *) container_of( nb, struct s3c24xx_i2c, freq_transition );  
	__u32				pclk_i2cclk_div_min=16*2;
	__u32				pclk_i2cclk_div_max=512*16;
	struct s3c2410_platform_i2c	*pdata=s3c24xx_i2c_get_platformdata( i2c->dev );
	__u32				pclk_min=pclk_i2cclk_div_min * ((__u32) pdata->bus_freq);
	__u32				pclk_max=pclk_i2cclk_div_max * ((__u32) pdata->max_freq);
		...
		case CPUFREQ_PRECHANGE:
		{
			...
			if( (pclk_new > pclk_max) || (pclk_new < pclk_min) )
			{
				if( (pclk_old < pclk_max) || (pclk_old > pclk_min) )
				{
					...
					clk_disable(i2c->clk);
					printk( KERN_WARNING "i2c_s3c24xx: I2C Clock divider invalid. Disabled i2c clock.\n" );
				}
			}
			else
			{
				...
				if( pclk_new > pclk_old )
				{
					...
					iicon=readl( i2c->regs + S3C2410_IICCON ) & 0xFFFFFFF0;
					
					/* Need to set the divider now. */
					if( s3c24xx_i2c_getdivisor( i2c, pdata, &iicon, &final_freq, pclk_new ) )
					{
						printk( KERN_WARNING "i2c_s3c24xx: Can't get right clock divider. Assuming maximum.\n" );
						/* Max divider= PCLK/512, I2C divider=16. */
						iicon|=0x0000004F;
					}
		...
		case CPUFREQ_POSTCHANGE:
		{
			...
			if( (pclk_old > pclk_max) || (pclk_old < pclk_min) )
			{
				if( (pclk_new < pclk_max) || (pclk_new > pclk_min) )
				{
					...
					printk( KERN_WARNING "i2c_s3c24xx: I2C Clock divider valid again. Re-enabled i2c clock.\n" );
				}
			}
			break;
		}
...
static int i2c_freq_policy( struct notifier_block *nb, unsigned long val, void *data )
{
	struct cpufreq_policy		*policy = data;
	struct s3c24xx_i2c		*i2c=(struct s3c24xx_i2c *) container_of( nb, struct s3c24xx_i2c, freq_policy );  
	__u32				pclk_i2cclk_div_min=16*2;
	__u32				pclk_i2cclk_div_max=512*16;
	struct s3c2410_platform_i2c	*pdata=s3c24xx_i2c_get_platformdata( i2c->dev );
	__u32				pclk_min=pclk_i2cclk_div_min * ((__u32) pdata->bus_freq);
...
static int s3c24xx_i2c_probe(struct device *dev)
{
	...
	memset( &i2c->freq_transition, 0, sizeof( i2c->freq_transition ) );
	memset( &i2c->freq_policy, 0, sizeof( i2c->freq_policy ) );
	i2c->freq_transition.notifier_call = i2c_freq_transition;
	i2c->freq_transition.priority = CPUFREQ_ORDER_S3C24XX_I2C_PRIO;
	i2c->freq_policy.notifier_call = i2c_freq_policy;
	cpufreq_register_notifier(&i2c->freq_transition, CPUFREQ_TRANSITION_NOTIFIER);
	cpufreq_register_notifier(&i2c->freq_policy, CPUFREQ_POLICY_NOTIFIER);
#endif
 out:
	if (ret < 0)
...
static int s3c24xx_i2c_remove(struct device *dev)
{
	...
	cpufreq_unregister_notifier(&i2c->freq_policy, CPUFREQ_POLICY_NOTIFIER);
#endif
...
static struct device_driver s3c2412_i2c_driver = {
	.name		= "s3c2412-i2c",
	.bus		= &platform_bus_type,
	.probe		= s3c24xx_i2c_probe,
	.remove		= s3c24xx_i2c_remove,
...
static struct device_driver s3c2443_i2c_driver = {
	.name		= "s3c2443-i2c",
	.bus		= &platform_bus_type,
	.probe		= s3c24xx_i2c_probe,
	.remove		= s3c24xx_i2c_remove,
...
static struct device_driver s3c2450_i2c_driver = {
	.name		= "s3c2450-i2c",
	.bus		= &platform_bus_type,
	.probe		= s3c24xx_i2c_probe,
	.remove		= s3c24xx_i2c_remove,
...
static int __init i2c_adap_s3c_init(void)
{
	...
	if (ret == 0)
		ret = driver_register(&s3c2412_i2c_driver); 
	if (ret == 0)
		ret = driver_register(&s3c2440_i2c_driver); 
	if (ret == 0)
		ret = driver_register(&s3c2443_i2c_driver); 
	if (ret == 0)
		ret = driver_register(&s3c2450_i2c_driver); 
...
static void __exit i2c_adap_s3c_exit(void)
{
	...
	driver_unregister(&s3c2412_i2c_driver);
	driver_unregister(&s3c2440_i2c_driver);
	driver_unregister(&s3c2443_i2c_driver);
	driver_unregister(&s3c2450_i2c_driver);
}
