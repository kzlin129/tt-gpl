...
#if defined(CONFIG_CPU_S3C2443) & defined(CONFIG_CPU_S3C2412)
...
static void iis_2443_setup_epll_path( void )
{
	/* Select SELI2s == 0	*/
	unsigned long reg = readl( S3C2443_CLKSRC );
	reg &= ~(S3C2412_CLKSRC_SELEREF_MASK);
	reg |= (1<<6);	/* SELEPLL instead of EpllRefClk	*/
	writel(reg, S3C2443_CLKSRC); 
			
	reg = readl(S3C2443_SCLKCON);
	reg |= S3C2443_SCLKCON_IISCLK;
	writel( reg, S3C2443_SCLKCON );
}

static int iis_s3c2443_hw_init(void)
{
	int iismod = 0;
	
	switch( IO_GetCodecMaster() ){
		case GOCODECCFG_SLAVE:
			iismod &= ~S3C2443_IISMOD_CDCLKCON;	/* specify to output S3c generated clk signal	*/
			iismod |= S3C2412_IISMOD_EMASTER;

			s3c2443_epll_set_rate( 32769231 );
			iis_clockrate = 32769231;
			msleep( 10 );
			
			/* We already made sure that we own the EPLL in iis.c	*/
			/* because parent of iis clock is EPLL 					*/ 
			iis_2443_setup_epll_path();
		...
		default:
			iismod |= S3C2443_IISMOD_CDCLKCON;	/* Codec is externally clocked        */
			iismod |= S3C2412_IISMOD_SLAVE;
			break;
	}
...
static void iis_s3c2443_hw_exit(void)
{
...
static int iis_s3c2443_set_samplerate(unsigned rate)
{
	unsigned long flags;
	
	PK_DBG("iis-2443@%u MHz (div=%u)\n", iis_clockrate, iis_clk_div);
	...
	switch( IO_GetCodecMaster() ){
		case GOCODECCFG_INTERNAL_MASTER:			
			...
			writel( ((iis_clk_div-1) &0x3ff) | S3C2412_IISPSR_PSRAEN, S3C2412_IISPSR); 
		...
		default:
			...
			case 22050:
			case 44100:	
				s3c2443_epll_set_rate( 67714286 );
			...
			case 24000:
			case 48000:	
				s3c2443_epll_set_rate( 73714286 );
			...
			writel(div << 0 | S3C2412_IISPSR_PSRAEN, S3C2412_IISPSR);
...
/**
 *  The S3C2443 I2S interface is basically a slightly modified S3C2412 interface.
...
iis_hwintf_t iis_s3c2443_intf = {
	.hw_init        = iis_s3c2443_hw_init,
	.hw_exit        = iis_s3c2443_hw_exit,
	.control_cmd    = iis_s3c2412_control_cmd,
	.set_samplerate = iis_s3c2443_set_samplerate,
