...
static uint16_t wm8971_current[43] = {
	0,
	0,
	0,
	0,
...
static inline uint16_t WM8971_RDREG(uint16_t reg)
{
	return wm8971_current[reg];
}

static void wm8971_init_registers(void)
{
	int i;
	uint16_t r;

	WM8971_WRREG(15, 0);
	for (i = 0; i < ARRAY_SIZE(wm8971); i++) {
		if (i != 15) {
			if ((r = wm8971[i]) != NO_INIT)
			{
...
static int wm8971_set_volume_dB(unsigned int volume)
{
	if (volume < 90) 
		volume = 121 - volume;
	else
	...
	WM8971_WRREG(40, 0x180 | volume);
	WM8971_WRREG(41, 0x180 | volume);
	WM8971_WRREG(42, 0x80 | volume);

	return 0;
}

static int wm8971_set_volume_internal(unsigned int volume)
{
	if (volume > 127)
		volume = 127;
...
static int wm8971_set_volume_external(unsigned int volume)
{
	if (volume > 127)
		volume = 127;

	WM8971_WRREG(2, 0x180 | volume);
	WM8971_WRREG(3, 0x180 | volume);

	return 0;
}

static int wm8971_set_volume_external_left(unsigned int volume)
{
	if (volume > 127)
		volume = 127;
...
static int wm8971_set_volume_external_right(unsigned int volume)
{
	if (volume > 127)
		volume = 127;
...
static int wm8971_set_volume(unsigned int volume)
{
	wm8971_set_volume_internal(volume);
	wm8971_set_volume_external(volume);

	return 0;
}

static int wm8971_set_treble(unsigned int treble)
{
	WM8971_WRREG(13, treble);

	return 0;
}

static int wm8971_set_bass(unsigned int bass)
{
	WM8971_WRREG(12, bass ^ 0x80);

	return 0;
}

static int wm8971_set_fixed12(unsigned int samplerate, uint16_t *newr8) 
{
...
static int wm8971_set_switch1211(unsigned int samplerate, uint16_t *newr8) 
{
...
static int wm8971_switch_line_in(unsigned int line_in)
{
	unsigned int value ;

	line_in = ((line_in & 1) << 6);
	value = WM8971_RDREG(32);
	value = (value & ~(0xc0)) | line_in;
	WM8971_WRREG(32, value);
	value = WM8971_RDREG(33);
	value = (value & ~(0xc0)) | line_in;
	WM8971_WRREG(33, value);

	return 0;
}

static int wm8971_set_register(unsigned int data)
{
	unsigned int reg = data >> 9;
	unsigned int value = (0x1ff & data);

	PK_DBG("SetRegister: %u => 0x%03x\n", reg, value);
	WM8971_WRREG(reg, value);
	return 0;
}

static void wm8971_powerdown(void)
{
...
void wm8971_init(struct codec_ops *ops)
{
	ops->set_volume = wm8971_set_volume;
	ops->set_volume_internal = wm8971_set_volume_internal;
	ops->set_volume_external = wm8971_set_volume_external;
	ops->set_register = wm8971_set_register;
	ops->resume = wm8971_init_registers;
	ops->suspend = wm8971_powerdown;
	ops->set_treble = wm8971_set_treble;
	ops->set_bass = wm8971_set_bass;
	ops->set_fs = wm8971_set_fs;
	ops->set_volume_external_left = wm8971_set_volume_external_left;
	ops->set_volume_external_right = wm8971_set_volume_external_right;
	ops->switch_line_in = wm8971_switch_line_in;
	ops->set_volume_dB = wm8971_set_volume_dB;

	wm8971_init_registers();
}
