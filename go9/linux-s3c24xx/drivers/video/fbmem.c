...
int
fb_screensave(struct fb_info *info, int mode)
{
	int ret = -EINVAL;

	printk(KERN_INFO "FB_SCREENSAVE(mode=%d)\n",mode);
	if (info->fbops->fb_screensave)
 		ret = info->fbops->fb_screensave(mode, info);
	 ...
	 unsigned long arg)
{
	...
	case FBIOSCREENSAVE:
		if (copy_from_user(&mode, argp, sizeof(mode)))
			return -EFAULT;
		return fb_screensave(info, mode);
