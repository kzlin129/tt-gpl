...
*/
#ifndef __INCLUDE_BARCELONA_USBMODE_H
#define __INCLUDE_BARCELONA_USBMODE_H
...
#define INPUT_BUS_POWER_ON_EVENT (1 << 0)
#define INPUT_BUS_POWER_OFF_EVENT (1 << 1)
#define INPUT_DEVICE_RESET (1 << 2 )
#define INPUT_DEVICE_DETECT_TIMEOUT (1 << 3 )
/* HOST == when we are host */
#define INPUT_HOST_DETECT_TIMEOUT (1 << 4 )
#define INPUT_HOST_DEVICE_CONNECT (1 << 5 )
#define INPUT_HOST_DEVICE_DISCONNECT (1 << 6 )
...
struct usbmode_operations {
	...
	int (*idle_to_initial) (struct usbmode_data *data);
	...
	int (*device_detect_to_initial) (struct usbmode_data *data);
	int (*device_detect_to_device) (struct usbmode_data *data);
	int (*device_to_initial) (struct usbmode_data *data);
	...
	int (*host_detect_to_initial) (struct usbmode_data *data);
	...
	int (*cla_to_initial) (struct usbmode_data *data);
	int (*cla_to_idle) (struct usbmode_data *data);
	int (*host_to_initial) (struct usbmode_data *data);
	int (*host_to_idle) (struct usbmode_data *data);

	/* usb suspend */
	void (*suspend_usb) (struct usbmode_data *data);

	/* power monitor */
	void (*start_powermon) (struct usbmode_data *data);
	void (*stop_powermon) (struct usbmode_data *data);
};
...
struct usbmode_data {
    ...
    /** while in suspend we don't handle events **/
    atomic_t event_barrier;
};
...
typedef void (* usb_state_change_listener_func)(USB_STATE previous_usb_state,USB_STATE current_usb_state, void* arg);
 ...
 **/
extern int add_usb_state_change_listener(usb_state_change_listener_func function, void* arg);

extern int remove_usb_state_change_listener(usb_state_change_listener_func function, void * arg);

#endif /* __INCLUDE_BARCELONA_USBMODE_H */
