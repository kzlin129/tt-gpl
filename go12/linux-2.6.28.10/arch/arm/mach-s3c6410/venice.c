/* linux/arch/arm/mach-s3c6410/venice.c
 *
...
static struct s3c2410_uartcfg venice_uartcfgs[] __initdata = {
	[0] = {
		.hwport	     = 0,
		.flags	     = 0,
...
// Pin tables
static struct vgpio_pin venice_vgpio0_pins[] = {
	...
	TT_DEF_PIN0(RFS_BOOT_CLK,	S3C64XX_GPK(2)),
	...
	TT_DEF_INVPIN0(GPS_1PPS, 	S3C64XX_GPN(10)),
	TT_DEF_INVPIN0(BT_RST,		S3C64XX_GPC(7)),
	TT_DEF_PIN0(ID_PIN,		S3C64XX_GPC(6)),
	...
	TT_DEF_PIN0(GSM_SYS_RST,	S3C64XX_GPL(1)),
	TT_DEF_NCPIN0(LCM_CS),
	TT_DEF_INVPIN0(LCM_PWR_EN,      S3C64XX_GPL(2)),
	TT_DEF_PIN0(LCM_ID,		S3C64XX_GPK(1)),
	...
	TT_DEF_PIN0(DOCK_I2C_EN,	S3C64XX_GPK(4)),
	TT_DEF_PIN0(ACCESSORY_PWR_EN,	S3C64XX_GPN(9)),
	TT_DEF_NCPIN0(DOCK_DET_PWR),
	TT_DEF_INVPIN0(HPDETECT,	S3C64XX_GPN(5)),
	TT_DEF_PIN0(TSP_ATTN,		S3C64XX_GPL(14)),
	TT_DEF_PIN0(TSP_CE,			S3C64XX_GPL(13)),
	TT_DEF_PIN0(TSP_PWR,		S3C64XX_GPL(6)),

	TT_DEF_PIN0(DR_PWR_EN,		S3C64XX_GPK(5)),
	TT_DEF_PIN0(KXR94_CS,		S3C64XX_GPK(12)),
	TT_DEF_PIN0(BARO_CS,		S3C64XX_GPM(2)),

	TT_DEF_PIN0(PWR_KILL,		S3C64XX_GPL(9)),
};

// Platform data & device
static struct vgpio_platform_data venice_vgpio_pdata[] = {
	[0] = {
        	.gpio_base      = TT_VGPIO0_BASE,
        	.gpio_number    = ARRAY_SIZE(venice_vgpio0_pins),
        	.pins           = venice_vgpio0_pins,
	},
};

static struct platform_device venice_vgpio[] = {
	[0] = {
        	.name           = "vgpio",
        	.id             = 0,
        	.dev            = {
                	.platform_data  = &venice_vgpio_pdata[0],
        	},
	},
};

/* Initializes OTG Phy. */
static void venice_otg_phy_init(void)
{
	int err;
...
/* OTG PHY Power Off */
static void venice_otg_phy_off(void) {
...
static struct s3c_otg_platform_data venice_otg = {
	.otg_phy_init   = venice_otg_phy_init,
	.otg_phy_off    = venice_otg_phy_off,
};
...
static void venice_quirk_setup(void)
{
	u32 cfg;
	...
	writel(cfg, S3C64XX_SPC_BASE);
        ...
        /* Setup UART0 for GPRS with flow control */
        s3c_gpio_cfgpin(S3C64XX_GPA(0), S3C_GPIO_SFN(2));
        s3c_gpio_cfgpin(S3C64XX_GPA(1), S3C_GPIO_SFN(2));
        s3c_gpio_cfgpin(S3C64XX_GPA(2), S3C_GPIO_SFN(2));
        s3c_gpio_cfgpin(S3C64XX_GPA(3), S3C_GPIO_SFN(2));
}

static struct i2c_board_info venice_i2c_devs0[] __initdata = {
	{ I2C_BOARD_INFO("24c01", 0x50), },
};

static struct platform_device *venice_devices[] /* __initdata */ = {
...
static void __init venice_setup_clocks(void)
{
	/* AR1520 is directly connected to RTC clock, so no need
	 * to use the GPF14 hack to supply it with the right clock.
	 */
}

/* QoS settings, same as SMDK... */
static void venice_set_qos(void)
{
	u32 reg;
...
static void __init venice_map_io(void)
{
	s3c64xx_init_io(venice_iodesc, ARRAY_SIZE(venice_iodesc));
	s3c24xx_init_clocks(XTAL_FREQ);
	s3c24xx_init_uarts(venice_uartcfgs, ARRAY_SIZE(venice_uartcfgs));

	s3c64xx_reserve_bootmem();
}

#ifdef CONFIG_SPI
extern void venice_spi_setup(void);
#endif

static void __init venice_machine_init(void)
{
	venice_quirk_setup();
	platform_device_register(&venice_vgpio[0]);
	...
	mendoza_s3cfb_setup("lms500hf04");
	mendoza_sdhci_setup();
#ifdef CONFIG_SPI
	venice_spi_setup();
#endif

	s3c_device_usbgadget.dev.platform_data = &venice_otg;
	s3c_device_usb_otghcd.dev.platform_data = &venice_otg;

	mendoza_ltc3577_i2c_init();
	i2c_register_board_info(0, venice_i2c_devs0, ARRAY_SIZE(venice_i2c_devs0));
	mendoza_i2c_setup(1);
	platform_add_devices(venice_devices, ARRAY_SIZE(venice_devices));
	tomtom_bl_setup();
...
MACHINE_START(VENICE, "Venice")
	...
	.map_io		= venice_map_io,
	.init_machine	= venice_machine_init,
	.timer		= &s3c64xx_timer,
MACHINE_END
