/* linux/arch/arm/mach-s3c6410/seoul.c
 *
...
static struct s3c2410_uartcfg seoul_uartcfgs[] __initdata = {
	[0] = {
		.hwport	     = 0,
		.flags	     = 0,
...
// Pin tables
static struct vgpio_pin seoul_vgpio0_pins[] = {
	TT_DEF_PIN0(PU_I2C0,		S3C64XX_GPK(3)),
	TT_DEF_PIN0(PU_I2C1,		S3C64XX_GPL(8)),
	TT_DEF_PIN0(BOOT_DEV0,		S3C64XX_GPN(13)),
//TT_DEF_PIN0(SD0_PWR_EN,		S3C64XX_GPL(11)),
	TT_DEF_PIN0(SD0_PWR_EN,		S3C64XX_GPM(5)),
	TT_DEF_PIN0(BARRACUDA_CS,	S3C64XX_GPA(7)),
	TT_DEF_PIN0(BACKLIGHT_EN,	S3C64XX_GPF(13)),
	TT_DEF_PIN0(LCM_RESET,		S3C64XX_GPL(3)),
	TT_DEF_PIN0(PWR_BUTTON,		S3C64XX_GPN(6)),
#if 0
	TT_DEF_PIN0(RFS_BOOT_CLK,	S3C64XX_GPB(4)),
	TT_DEF_PIN0(RFS_BOOT_Q,		S3C64XX_GPR(3)),
#endif
	TT_DEF_PIN0(USB_HOST_DETECT,	S3C64XX_GPN(2)),
	TT_DEF_INVPIN0(GPS_STANDBY,	S3C64XX_GPL(12)),
	TT_DEF_INVPIN0(GPS_RESET, 	S3C64XX_GPP(13)),
	TT_DEF_INVPIN0(BT_RST,		S3C64XX_GPO(5)),
	TT_DEF_PIN0(WALL_ON,		S3C64XX_GPK(1)),
	...
	TT_DEF_PIN0(LCM_CS,		S3C64XX_GPC(3)),
	TT_DEF_PIN0(LCM_PWR_EN,		S3C64XX_GPL(2)),
	TT_DEF_PIN0(CODEC_PWR_EN,	S3C64XX_GPK(10)),
	TT_DEF_PIN0(AMP_PWR_EN,		S3C64XX_GPK(9)),
	TT_DEF_PIN0(MIC_STBY,		S3C64XX_GPP(3)),
	TT_DEF_INVPIN0(DOCK_RESET,	S3C64XX_GPB(4)),
	TT_DEF_PIN0(DOCK_DET0,		S3C64XX_GPN(5)),
	TT_DEF_PIN0(DOCK_DET1,		S3C64XX_GPN(7)),
	TT_DEF_PIN0(ACCESSORY_PWR_EN,	S3C64XX_GPN(9)),
	TT_DEF_NCPIN0(DOCK_DET_PWR),
	TT_DEF_INVPIN0(HPDETECT,	S3C64XX_GPM(0)),
};

// Platform data & device
static struct vgpio_platform_data seoul_vgpio_pdata[] = {
	[0] = {
        	.gpio_base      = TT_VGPIO0_BASE,
        	.gpio_number    = ARRAY_SIZE(seoul_vgpio0_pins),
        	.pins           = seoul_vgpio0_pins,
	},
};

static struct platform_device seoul_vgpio[] = {
	[0] = {
        	.name           = "vgpio",
        	.id             = 0,
        	.dev            = {
                	.platform_data  = &seoul_vgpio_pdata[0],
        	},
	},
};

/* Initializes OTG Phy. */
static void seoul_otg_phy_init(void)
{
	int err;
...
/* OTG PHY Power Off */
static void seoul_otg_phy_off(void) {
...
static struct s3c_otg_platform_data seoul_otg = {
	.otg_phy_init   = seoul_otg_phy_init,
	.otg_phy_off    = seoul_otg_phy_off,
};

/* First setup whatever oddity the HW requires... */
static void seoul_quirk_setup(void)
{
}

static struct i2c_board_info seoul_i2c_devs0[] __initdata = {
};

static struct i2c_board_info seoul_i2c_devs1[] __initdata = {
};

static struct platform_device *seoul_devices[] /* __initdata */ = {
...
static void __init seoul_setup_clocks(void)
{
#if 0
...
static void __init seoul_map_io(void)
{
	s3c64xx_init_io(seoul_iodesc, ARRAY_SIZE(seoul_iodesc));
	s3c24xx_init_clocks(XTAL_FREQ);
	s3c24xx_init_uarts(seoul_uartcfgs, ARRAY_SIZE(seoul_uartcfgs));

	s3c64xx_reserve_bootmem();
}

void seoul_spi_setup(void);

static void __init seoul_machine_init(void)
{
	platform_device_register(&seoul_vgpio[0]);
	...
	s3c_device_usbgadget.dev.platform_data = &seoul_otg;
	s3c_device_usb_otghcd.dev.platform_data = &seoul_otg;

	i2c_register_board_info(0, seoul_i2c_devs0, ARRAY_SIZE(seoul_i2c_devs0));
	i2c_register_board_info(1, seoul_i2c_devs1, ARRAY_SIZE(seoul_i2c_devs1));
	mendoza_i2c_setup(3);
	platform_add_devices(seoul_devices, ARRAY_SIZE(seoul_devices));
	tomtom_bl_setup();
...
MACHINE_START(SEOUL, "Seoul")
	...
	.map_io		= seoul_map_io,
	.init_machine	= seoul_machine_init,
	.timer		= &s3c64xx_timer,
MACHINE_END
