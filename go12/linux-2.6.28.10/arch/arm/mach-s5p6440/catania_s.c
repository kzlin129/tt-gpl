/* linux/arch/arm/mach-s5p6440/catania.c
 *
...
static struct s3c2410_uartcfg catania_uartcfgs[] __initdata = {
	[0] = {
		.hwport	     = 0,
		.flags	     = 0,
...
struct map_desc catania_iodesc[] = {};
...
// Pin tables
static struct vgpio_pin catania_vgpio0_pins[] = {
	TT_DEF_PIN0(PU_I2C1,		S5P64XX_GPR(0)),
	TT_DEF_PIN0(WALL_ON,		S5P64XX_GPP(3)),
	TT_DEF_INVPIN0(HRESET,		S5P64XX_GPR(1)),
	...
	TT_DEF_PIN0(AMP_PWR_EN,		S5P64XX_GPP(4)),
	TT_DEF_INVPIN0(GPS_RESET, 	S5P64XX_GPR(2)),
	TT_DEF_INVPIN0(GPS_1PPS, 	S5P64XX_GPN(9)),
	TT_DEF_INVPIN0(BT_RST,		S5P64XX_GPN(11)),
	TT_DEF_PIN0(SD_BASE,		S5P64XX_GPG(0)),
	TT_DEF_PIN0(SD_BOOT_BASE,	S5P64XX_GPH(0)),
	TT_DEF_INVPIN0(MIC_RESET,	S5P64XX_GPF(15)),
	TT_DEF_PIN0(LCD_SPI_CS,	S5P64XX_GPR(5)),
	TT_DEF_PIN0(LCD_SPI_SDI,	S5P64XX_GPC(2)),
	TT_DEF_PIN0(LCD_SPI_SCL,	S5P64XX_GPC(1)),
	TT_DEF_PIN0(TILT_OUT,		S5P64XX_GPN(7)),
	TT_DEF_PIN0(TILT_PWR,		S5P64XX_GPN(8)),
...
static struct vgpio_platform_data catania_vgpio_pdata[] = {
	[0] = {
        	.gpio_base      = TT_VGPIO0_BASE,
        	.gpio_number    = ARRAY_SIZE(catania_vgpio0_pins),
        	.pins           = catania_vgpio0_pins,
	},
};

static struct platform_device catania_vgpio[] = {
	[0] = {
        	.name           = "vgpio",
        	.id             = 0,
        	.dev            = {
                	.platform_data  = &catania_vgpio_pdata[0],
        	},
	},
};

/* First setup whatever oddity the HW requires... */
static void catania_quirk_setup(void)
{
...
static struct i2c_board_info catania_i2c_devs0[] __initdata = {
};

static struct i2c_board_info catania_i2c_devs1[] __initdata = {
	{ I2C_BOARD_INFO("24c01", 0x50), },
};

static struct platform_device *catania_devices[] /* __initdata */ = {
...
/* Initializes OTG Phy. */
static void catania_otg_phy_init(void)
{
...
/* OTG PHY Power Off */
static void catania_otg_phy_off(void)
{
...
static struct s3c_otg_platform_data catania_otg = {
	.otg_phy_init	= catania_otg_phy_init,
	.otg_phy_off	= catania_otg_phy_off,
};
...
static void __init catania_setup_clocks(void)
{

}

static void catania_wakeup_irq(int irq)
{
	pr_info("Woke-up from IRQ#%d\n", irq);
}

static void __init catania_map_io(void)
{
	s5p64xx_init_io(catania_iodesc, ARRAY_SIZE(catania_iodesc));
	s3c24xx_init_clocks(XTAL_FREQ);
	s3c24xx_init_uarts(catania_uartcfgs, ARRAY_SIZE(catania_uartcfgs));

	s3c64xx_reserve_bootmem();
}

#ifdef CONFIG_SPI
extern void catania_spi_setup(void);
#endif

static void __init catania_machine_init(void)
{
	platform_device_register(&catania_vgpio[0]);
	catania_quirk_setup();

	pm_cpu_wakeup_irq = catania_wakeup_irq;

	i2c_register_board_info(0, catania_i2c_devs0, ARRAY_SIZE(catania_i2c_devs0));
	i2c_register_board_info(1, catania_i2c_devs1, ARRAY_SIZE(catania_i2c_devs1));
	mendoza_s5m8751_i2c_init(1); // 2nd bus 
	mendoza_i2c_setup(3);  

	s3c_adc_set_platdata(&s3c_adc_platform);
#ifdef CONFIG_TOMTOM_BATTERY
	mendoza_battery_adc_setup(AIN2, AIN0); // no charge current channel: use AIN0 (unused)
#endif
#ifdef CONFIG_TOUCHSCREEN_S3C
	s3c_ts_set_platdata(&s3c_ts_platform);
#endif
	mendoza_s3cfb_setup("a043fw05v4");
#ifdef CONFIG_SPI
        // AUO needs SPI
	catania_spi_setup();
#endif
	...
	s3c_device_usbgadget.dev.platform_data = &catania_otg;
	s3c_device_usb_otghcd.dev.platform_data = &catania_otg;

	platform_add_devices(catania_devices, ARRAY_SIZE(catania_devices));

	s5p6440_pm_init();

	catania_setup_clocks();
#ifdef CONFIG_TOMTOM_GPRS
	gprs_init();
#endif
}

MACHINE_START(CATANIA, "catania")
	...
	.map_io		= catania_map_io,
	.init_machine	= catania_machine_init,
	.timer		= &s3c64xx_timer,
MACHINE_END
