...
static int s3c_pwm_probe(struct platform_device *pdev)
{
	...
	if (id == 0) {
		...
		printk(KERN_ERR "This PWM doesn't support PWM out\n");
	}
...
static int s3c_pwm_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	printk(KERN_INFO"PWM: suspending ch%d\n",id);

	reg_tcon = __raw_readl(S3C2410_TCON);
...
{
	unsigned int id = pdev->id;
	printk(KERN_INFO"PWM: resuming ch%d\n",id);

	__raw_writel(reg_tcon,  S3C2410_TCON);
