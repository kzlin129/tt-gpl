...
static struct s3c_media_device s3c_mdevs[S3C_MDEV_MAX] = {
	...
	{
		.id = S3C_MDEV_GVG_EDGE,
		.name = "s3c-gvg",

#ifdef CONFIG_VIDEO_GVG_USE_BOOTMEM
		.memsize = CONFIG_VIDEO_GVG_MEMSIZE_EDGE * SZ_1K,
#else
		.memsize = 0,
#endif
		.paddr = 0,
	...
	{
		.id = S3C_MDEV_GVG_VERTEX,
		.name = "s3c-gvg",

#ifdef CONFIG_VIDEO_GVG_USE_BOOTMEM
		.memsize = CONFIG_VIDEO_GVG_MEMSIZE_VERTEX * SZ_1K,
#else
		.memsize = 0,
#endif
		.paddr = 0,
...
EXPORT_SYMBOL(s3c_get_media_device);
