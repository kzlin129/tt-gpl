...
static inline int s3c_gpio_do_setcfg_slp(struct s3c_gpio_chip *chip,
				     unsigned int off, unsigned int config)
{
	if (!chip->config_slp)
		return -EINVAL;

	return (chip->config_slp->set_config_slp)(chip, off, config);
}

static inline int s3c_gpio_do_setpull_slp(struct s3c_gpio_chip *chip,
				      unsigned int off, s3c_gpio_pull_t pull)
{
	if (!chip->config_slp)
		return -EINVAL;

	return (chip->config_slp->set_pull_slp)(chip, off, pull);
}
...
/**
 * s3c_gpio_setcfg_slp - S3C64XX/S5C64XX sleep GPIO configuration.
 * @chip: The gpio chip that is being configured.
 * @off: The offset for the GPIO being configured.
 * @cfg: The configuration value to set.
 *
 ...
 *	10 = previous state
*/
extern int s3c_gpio_setcfg_slp(struct s3c_gpio_chip *chip,
				unsigned int off, unsigned int cfg);

/**
 * s3c_gpio_setpull_updown_slp() - S3C64XX/S5C64XX Sleep pull configuration
 * @chip: The gpio chip that is being configured.
 * @off: The offset for the GPIO being configured.
 * @param: pull: The pull mode being requested.
 *
 ...
 * order:
 *	00 = No pull resistor connected
 *	01 = Pull-down resistor connected
 *	10 = Pull-up resistor connected
 *	11 = Reserved
 */
extern int s3c_gpio_setpull_updown_slp(struct s3c_gpio_chip *chip,
				   unsigned int off, s3c_gpio_pull_t pull);
