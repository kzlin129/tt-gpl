...
static void lms430_enable (struct clk *clk)
{
	gpio_set_value(TT_VGPIO_LCD_ON, 1);
}

static void lms430_disable (struct clk *clk)
{
	gpio_set_value(TT_VGPIO_LCD_ON, 0);
	panel_sleep(150);				/* T5-T4 should be greater than 6 frames */
}

static void lms430_suspend (struct clk *clk)
{
	u32 val;
...
static void lms430_resume (struct clk *clk)
{
...
static void lms430_init (struct clk *clk)
{
	gpio_set_value(TT_VGPIO_LCD_ON, 0);	/* Initially LCD_OFF in the begining of T2 */
}

static void lms430_setup (struct clk *clk)
{
	printk (KERN_INFO "Panel: LMS430\n");
...
static struct clcd_panel_cntrl cntrl = {
	.enable		= lms430_enable,
	.disable	= lms430_disable,
	.suspend	= lms430_suspend,
	.resume		= lms430_resume,
	.init		= lms430_init,
	.setup		= lms430_setup,
};
...
static struct clcd_device lms430 = {
	.timings = &timings,
	.cntrl 	 = &cntrl,
};

struct clcd_paneldev LMS430_WithPLL =
{
	.panel 		= &lms430,
	.lcd_mdiv	= 0x00000018,
};
