...
static void lms500_enable (struct clk *clk)
{
...
static void lms500_init (struct clk *clk)
{
	gpio_set_value(TT_VGPIO_LCD_ON, 0);	/* Initially LCD_OFF in the begining of T2 */
}

static void lms500_setup (struct clk *clk)
{
	printk (KERN_INFO "Panel: LMS500\n");
...
static struct clcd_panel_cntrl cntrl = {
	.enable		= lms500_enable,
	.disable	= lms500_disable,
	.resume		= lms500_resume,
	.suspend	= lms500_suspend,
	.init		= lms500_init,
	.setup		= lms500_setup,
};
...
static struct clcd_device lms500 = {
	.timings = &timings,
	.cntrl   = &cntrl,
};

struct clcd_paneldev LMS500_WithPLL =
{
	.panel 		= &lms500,
	.lcd_mdiv	= 0x00000018,
};
