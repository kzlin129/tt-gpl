			...
			dma_addr_t data, int size)
{
	...
	if ((chan->flags & S3C2410_DMAF_AUTOSTART)
			&& !(readl(chan->regs + PL080S_CH_CONFIG) & PL080_CONFIG_ENABLE))
...
#define JLOG_SIZE	25
static unsigned long jif[JLOG_SIZE];
static int jind = 0;

void s3c2410_dma_dump_dbginfo(void)
{
	int j = 0;
...
EXPORT_SYMBOL(s3c2410_dma_dump_dbginfo);
...
/* system device class */
#ifdef CONFIG_PM
static int s3c64xx_dma_suspend (struct sys_device *dev, pm_message_t state)
{
	return 0;
}

static int s3c64xx_dma_resume (struct sys_device *dev)
{
	struct s3c64xx_dmac * dmac = container_of(dev, struct s3c64xx_dmac, sysdev);
...
#else
#define s3c64xx_dma_suspend NULL
#define s3c64xx_dma_resume  NULL
#endif	/* CONFIG_PM */
