...
static struct s3c_gpio_cfg_slp gpio_cfg_slp = {
	.set_config_slp = s3c_gpio_setcfg_slp,
	.set_pull_slp   = s3c_gpio_setpull_updown_slp,
};

static int s3c_gpm_to_irq(struct gpio_chip *chip, unsigned offset)
{
	if (offset > 4)
		return -EINVAL;

	return IRQ_EINT(offset + 23);
}
...
static struct s3c_gpio_chip gpio_4bit[] = {
	{
		...
		.chip	= {
			...
			.to_irq	= s3c_gpm_to_irq,
			.label	= "GPM",
		},
	},
};

static int s3c_gpl_to_irq(struct gpio_chip *chip, unsigned offset)
{
	if (offset < 8)
		return -EINVAL;

	return IRQ_EINT(offset + 8);
}
