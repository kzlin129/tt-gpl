...
#include <linux/broadcom/vc03/vc_vchi_fileservice_defs.h>
...
typedef struct {
   ...
   //TODO probably only need 1 _msg
   //Host->2727
...
static char bulk_buffer[FILESERV_MAX_BULK]  __attribute__ ((aligned (16))) ;
...
#endif
static int fs_host_direntbytestream_create(struct vc03_dirent *d, void *buffer);
static void fs_host_direntbytestream_interp(struct vc03_dirent *d, void *buffer);
...
int vc_vchi_filesys_init (VCHI_INSTANCE_T initialise_instance, VCHI_CONNECTION_T **connections, uint32_t num_connections ) {
   ...
   vc_filesys_client.cur_xid = 0;
       ...
       SERVICE_CREATION_T filesys_parameters = { FILESERV_4CC,              // 4cc service code
                                                 connections[0],           // passed in fn ptrs
                                                 0,                        // tx fifo size (unused)
                                                 0,                        // tx fifo size (unused)
                                                 &filesys_callback,         // service callback
                                             &vc_filesys_client.filesys_msg_avail }; // callback parameter
...
static void filesys_task_func( unsigned argc, void *argv )
{
   ...
   while(1) {
      ...
      success = vc_fs_message_handler(&vc_filesys_client.vc_msg, msg_len);
   }
...
int vc_filesys_close(int fildes)
{
   ...
   os_semaphore_obtain( &vc_filesys_client.filesys_lock );
   vc_filesys_client.fileserv_msg.params[0] = fildes;
...
long vc_filesys_lseek(int fildes, long offset, int whence)
{
   ...
   os_semaphore_obtain( &vc_filesys_client.filesys_lock );
   vc_filesys_client.fileserv_msg.params[0] = fildes;
   vc_filesys_client.fileserv_msg.params[1] = offset;
   vc_filesys_client.fileserv_msg.params[2] = whence;
...
int64_t vc_filesys_lseek64(int fildes, int64_t offset, int whence)
{
   ...
   vc_filesys_client.fileserv_msg.params[3] = whence;
...
int vc_filesys_umount(const char *mountpoint)
{
   ...
   if (vchi_msg_stub(&vc_filesys_client.fileserv_msg, VC_FILESYS_UMOUNT, strlen(mountpoint)+1+16) != FILESERV_RESP_OK) {
      os_semaphore_release( &vc_filesys_client.filesys_lock );
      return -1;
   }
...
int vc_filesys_open(const char *path, int vc_oflag)
{
   ...
   vc_filesys_client.fileserv_msg.params[0] = vc_oflag;
   strncpy((char *)vc_filesys_client.fileserv_msg.data, path, FS_MAX_PATH);
   
   if (vchi_msg_stub(&vc_filesys_client.fileserv_msg, VC_FILESYS_OPEN, 16+(strlen((char *)vc_filesys_client.fileserv_msg.data)+1) )
         != FILESERV_RESP_OK) {
      os_semaphore_release( &vc_filesys_client.filesys_lock );
      return -1;
   }

   fd = vc_filesys_client.fileserv_msg.params[0];
...
static int vc_vchi_msg_bulk_read(FILESERV_MSG_T* msg, uint16_t cmd_id, uint32_t transfer_len, uint8_t* recv_addr )
{
   ...
   //required to align recv_addr to 16 byte
   host_align_bytes =  ((int)recv_addr&15) ? (16 - ((int)recv_addr&15)) : 0;
   ...
   vchi_msg_queue( vc_filesys_client.open_handle, msg, msg_len, VCHI_FLAGS_BLOCK_UNTIL_QUEUED, NULL );
   ...
   //copy vc_align bytes to recv_addr, now ready for bulk
   if(host_align_bytes) {
   ...
   if(msg->cmd_code == FILESERV_BULK_WRITE){
      ...
      uint32_t bulk_bytes = (num_bytes_read-host_align_bytes)&(~15);
      uint32_t end_bytes = num_bytes_read-host_align_bytes-bulk_bytes;
      
      assert(end_bytes <= 16);
      
      vchi_bulk_queue_receive(vc_filesys_client.open_handle,
                              recv_addr,
                              bulk_bytes,
...
int vc_filesys_read(int fildes, void *buf, unsigned int nbyte)
{
   ...
   //will split the read into 4K chunks based on host array size
   ...
   do {
      ...
      vc_filesys_client.fileserv_msg.params[0] = fildes;
      vc_filesys_client.fileserv_msg.params[1] = -1;        // offset: use -1 to indicate no offset
               
      actual_read = vc_vchi_msg_bulk_read(&vc_filesys_client.fileserv_msg , VC_FILESYS_READ, bulk_bytes, (uint8_t*)ptr); 
...
int vc_vchi_msg_bulk_write(FILESERV_MSG_T* msg, uint16_t cmd_id, uint32_t transfer_len, uint8_t* send_addr )
{
   ...
   if(transfer_len <= FILESERV_MAX_DATA) {
      ...
      assert(num_bytes_written == transfer_len);
      
        
   }
   else {
      //required to align recv_addr to 16 byte
      vc_align_bytes =  ((int)send_addr&15) ? (16 - ((int)send_addr&15)) : 0;
      
      //copy vc_align bytes to msg->data, send_addr ready for bulk
      if(vc_align_bytes) {
         msg->params[3] = vc_align_bytes;
         memcpy(msg->data, send_addr, vc_align_bytes);
         transfer_len -= vc_align_bytes;
         send_addr += vc_align_bytes; 
         msg_len += vc_align_bytes;
      }
      else 
         msg->params[3] = 0;
   
      bulk_bytes = (transfer_len)&(~15);
   
      bulk_end_bytes = transfer_len-bulk_bytes;
   
      if(bulk_end_bytes) {
         memcpy(msg->data+vc_align_bytes, send_addr+bulk_bytes, bulk_end_bytes);
      ...
      vchi_bulk_queue_transmit( vc_filesys_client.open_handle,
                                         send_addr,
                                         bulk_bytes,
   ...
   //we will make this more dynamic later
	do {   
   	bulk_bytes = nbyte > FILESERV_MAX_BULK ? FILESERV_MAX_BULK : nbyte;
     	...
     	actual_written = vc_vchi_msg_bulk_write(&vc_filesys_client.fileserv_msg , VC_FILESYS_WRITE, bulk_bytes, (uint8_t*)ptr); 
...
int vc_filesys_format(const char *path)
{
   ...
   if (vchi_msg_stub(&vc_filesys_client.fileserv_msg, VC_FILESYS_FORMAT,
                  16+strlen((char *)vc_filesys_client.fileserv_msg.data)+1)
         != FILESERV_RESP_OK) {
      os_semaphore_release( &vc_filesys_client.filesys_lock );
      return -1;
   }
...
int vc_filesys_get_attr(const char *path, fattributes_t *attr)
{
   ...
   *attr = vc_filesys_client.fileserv_msg.params[0];
   os_semaphore_release( &vc_filesys_client.filesys_lock );
   return 0;
}
...
struct vc03_dirent *vc_filesys_readdir_r(void *dhandle, struct vc03_dirent *result)
{
   os_semaphore_obtain( &vc_filesys_client.filesys_lock );
   vc_filesys_client.fileserv_msg.params[0] = (uint32_t)dhandle;
...
{
   os_semaphore_obtain( &vc_filesys_client.filesys_lock );
   vc_filesys_client.fileserv_msg.params[0] = attr;
   strncpy((char *)vc_filesys_client.fileserv_msg.data, path, FS_MAX_PATH);
...
void vc_filesys_scandisk(const char *path)
{
   os_semaphore_obtain( &vc_filesys_client.filesys_lock );
   strncpy((char *)vc_filesys_client.fileserv_msg.data, path, FS_MAX_PATH);
   vchi_msg_stub(&vc_filesys_client.fileserv_msg, VC_FILESYS_SCANDISK, 16+strlen((char *)vc_filesys_client.fileserv_msg.data)+1);
   os_semaphore_release( &vc_filesys_client.filesys_lock );
   return;
}
...
int vc_filesys_diskwritable(const char *path)
{
   ...
   writable = vc_filesys_client.fileserv_msg.params[0];
   os_semaphore_release( &vc_filesys_client.filesys_lock );
   return writable;
}
...
int vc_filesys_fstype(const char *path)
{
   ...
   fstype = vc_filesys_client.fileserv_msg.params[0];
   os_semaphore_release( &vc_filesys_client.filesys_lock );
   return fstype;
}
...
int vc_filesys_open_disk_raw(const char *path)
{
   ...
   if (vchi_msg_stub(&vc_filesys_client.fileserv_msg, VC_FILESYS_OPEN_DISK_RAW, 16+strlen((char *)vc_filesys_client.fileserv_msg.data)+1) == FILESERV_RESP_OK)
      retval = vc_filesys_client.fileserv_msg.params[0];
...
int vc_filesys_close_disk_raw(const char *path)
{
   ...
   if (vchi_msg_stub(&vc_filesys_client.fileserv_msg, VC_FILESYS_CLOSE_DISK_RAW, 16+strlen((char *)vc_filesys_client.fileserv_msg.data)+1) == FILESERV_RESP_OK)
      retval = vc_filesys_client.fileserv_msg.params[0];
...
int vc_filesys_open_disk(const char *path)
{
   ...
   if (vchi_msg_stub(&vc_filesys_client.fileserv_msg, VC_FILESYS_OPEN_DISK, 16+strlen((char *)vc_filesys_client.fileserv_msg.data)+1) == FILESERV_RESP_OK)
      retval = vc_filesys_client.fileserv_msg.params[0];
...
int vc_filesys_close_disk(const char *path)
{
   ...
   if (vchi_msg_stub(&vc_filesys_client.fileserv_msg, VC_FILESYS_CLOSE_DISK, 16+strlen((char *)vc_filesys_client.fileserv_msg.data)+1) == FILESERV_RESP_OK)
      retval = vc_filesys_client.fileserv_msg.params[0];
...
int vc_filesys_numsectors(const char *path)
{
   ...
   if (vc_fs_stub(VC_FILESYS_NUMSECTORS, 16+strlen((char *)vc_fsg.host_body.data)+1) == FILESERV_RESP_OK)
...
int vc_filesys_errno(void)
{
   return vc_filesys_client.err_no;
}
...
static int vchi_msg_stub(FILESERV_MSG_T* msg, uint16_t cmd_id, int msg_len )
{
    ...
    success += vchi_msg_queue( vc_filesys_client.open_handle, msg, msg_len, VCHI_FLAGS_BLOCK_UNTIL_QUEUED, NULL );
...
static int vc_send_response( FILESERV_MSG_T* msg, uint32_t retval, uint32_t nbytes )
{
   ...
   assert(nbytes <= 1024);
   success = vchi_msg_queue( vc_filesys_client.open_handle, msg, nbytes, VCHI_FLAGS_BLOCK_UNTIL_QUEUED, NULL );
...
static int vc_fs_message_handler( FILESERV_MSG_T* msg, uint32_t nbytes )
{
   ...
   if (xid == vc_filesys_client.cur_xid) {
      ...
      vc_filesys_client.resp_code = (int)vc_filesys_client.fileserv_msg.cmd_code;
           
      if (vc_filesys_client.resp_code == FILESERV_RESP_ERROR) {
         vc_filesys_client.err_no = (int)vc_filesys_client.fileserv_msg.params[0];
      }
            
       // signal vchi_msg_stub which will be waiting for response
       if ( os_semaphore_obtained(&vc_filesys_client.response_lock) ) 
         os_semaphore_release( &vc_filesys_client.response_lock );
     	 else
         assert(0);
      ...
      case VC_FILESYS_CLOSE:
         ...
         i = vc_hostfs_close(msg->params[0]);
         if (i != 0) {
      ...
      case VC_FILESYS_LSEEK:
         ...
         i = vc_hostfs_lseek(msg->params[0],
                             msg->params[1],
                             msg->params[2]);
         if (i < 0) {
            retval = FILESERV_RESP_ERROR;
      ...
      case VC_FILESYS_LSEEK64:
      {
         ...
         offset = vc_hostfs_lseek64(msg->params[0],offset,msg->params[3]);
         if (offset < (int64_t)0) {
            retval = FILESERV_RESP_ERROR;
            rlen = 0;
         } else {
      ...
      case VC_FILESYS_READ:
      {
         ...
         uint32_t nbyte_offset = 0;
         uint32_t num_resp_bufs = 0;
         uint32_t nbyte_read = 0;
         uint32_t end_bytes = 0;
         vc_assert(((int)bulk_buffer & 15) == 0);
         vc_assert(total_bytes <= sizeof(bulk_buffer)); 
         rlen = 0;
         ...
         //bulk transfer required
            ...
            //we send the bytes required for VC buffer align
            if(nalign_bytes) {
               i = vc_hostfs_read(fd,
                               msg->data,
                               nalign_bytes);
            ...
            //bulk bytes
            bulk_bytes = vc_hostfs_read(fd,bulk_buffer,total_bytes);
            
            if(bulk_bytes <= FILESERV_MAX_DATA) {         	
            	memcpy(&msg->data[nalign_bytes], &bulk_buffer[0], bulk_bytes);
            	//all data will be in one msg
               retval = FILESERV_RESP_OK;
               //read size
               msg->params[0] = (bulk_bytes + nalign_bytes);
               msg->params[1] = 0;
            	rlen = bulk_bytes + nalign_bytes + 16;
            	//send response 
               vc_send_response( msg, retval, rlen );
               return(1);   	
            }
            ...
            //copy end unaligned bytes into msg->data
            end_bytes  = (bulk_bytes & 15);
            if(end_bytes) {
               int end_index = bulk_bytes - end_bytes;
         ...
         //we don't need to send anymore msg so return
         return(1);
      }
      
      case VC_FILESYS_READDIR:
      {
         struct vc03_dirent result;
      ...
      case VC_FILESYS_SETEND:
         ...
         i = vc_hostfs_setend(msg->params[0]);
         if (i != 0) {
            retval = FILESERV_RESP_ERROR;
         }
         rlen = 0;
