...
/******************************************************************************
NAME
   vc_msgfifo_read_header

ARGUMENT
   interface number, pointer to the FIFO header struct
FUNCTION
   Reads VC_INTERFACE_BLOCK_SIZE bytes from input fifo as a fifo msg header.
   This function may block while it waits for enough data to arrive.

RETURNS
   -
******************************************************************************/
#ifdef REQUIRE_VCOGLES
//Only used by ogles at the moment
void vc_msgfifo_read_header(int inum, VC_MSGFIFO_CMD_HEADER_T *header, void *event) {
   //Read header, blocks if there is not enough data
   vc_msgfifo_read_blocking(inum, header, VC_INTERFACE_BLOCK_SIZE, event);
   //Then swap round the fields if necessary
   header->sync = VC_VTOH32(header->sync);
   header->xid = VC_VTOH32(header->xid);
   header->cmd_code = VC_VTOH32(header->cmd_code);
   header->ext_length = VC_VTOH16(header->ext_length);
   header->timestamp = VC_VTOH16(header->timestamp);
   //Do we need to flush the read fifo??
}
#endif
