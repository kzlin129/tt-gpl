...
non_wrap_fifo_create( const char *name, const uint32_t size, int alignment, int no_of_slots )
{
   ...
   fifo = os_malloc( sizeof(NON_WRAP_FIFO_HANDLE_T), OS_ALIGN_DEFAULT, "" );
   os_assert( fifo );
   if (!fifo) return NULL;
   ...
   if ( fifo->malloc_address ) {
      ...
      fifo->slot_info = os_malloc(no_of_slots*sizeof(NON_WRAP_SLOT_T), OS_ALIGN_DEFAULT, "");
...
non_wrap_fifo_request_write_address( VCHI_NWFIFO_T *_fifo, void **address, uint32_t size, int block )
{
   ...
   if ( success == 0 ) {
      ...
      fifo->slot_info[fifo->write_slot].address = *address;
