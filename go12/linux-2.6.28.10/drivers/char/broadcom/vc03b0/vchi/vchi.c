...
int32_t success;
   ...
   ptr->state = function_table->init(low_level);
   return(ptr);
}
...
int32_t success = -1;
   ...
   success = vchi_bulk_aux_service_init( connections, num_connections );
   os_assert( success >= 0 );

   // Initialise the control service on each of the connections
   // This will wait for the other side to respond before returning
   success = vchi_control_service_init( connections, num_connections );
   os_assert( success >= 0 );
...
int32_t vchi_service_destroy( const VCHI_SERVICE_HANDLE_T service )
{
   ...
   if(( service_details != NULL ) && ((service_details->state & SERVER) == SERVER))
   {
...
int32_t vchi_service_close( const VCHI_SERVICE_HANDLE_T handle )
{
   ...
   if( service != NULL && ((service->state & SERVER) == CLIENT) )
   {
 ...
 *
 ***********************************************************/
int32_t vchi_msg_queue( const VCHI_SERVICE_HANDLE_T handle,
 ...
 *
 ***********************************************************/
int32_t vchi_msg_queuev( const VCHI_SERVICE_HANDLE_T handle,
                         VCHI_MSG_VECTOR_T * const vector,
 ...
 *
 ***********************************************************/
int32_t vchi_msg_dequeue( const VCHI_SERVICE_HANDLE_T handle,
                          void *data,
 ...
 *
 ***********************************************************/
extern int32_t vchi_msg_peek( const VCHI_SERVICE_HANDLE_T handle,
                              void **data,
                              uint32_t *msg_size,
                              VCHI_FLAGS_T flags )
{
 ...
 *
 ***********************************************************/
extern int32_t vchi_msg_hold( const VCHI_SERVICE_HANDLE_T handle,
                              void **data,
                              uint32_t *msg_size,
                              VCHI_FLAGS_T flags,
                              VCHI_HELD_MSG_T *message_handle )
 ...
 *
 ***********************************************************/
extern int32_t vchi_msg_remove( const VCHI_SERVICE_HANDLE_T handle )
{
   int32_t success = -1;

   VCHI_HELD_MSG_T msg;
   success = vchi_msg_hold( handle, NULL, NULL, VCHI_FLAGS_NONE, &msg );
   if (success == 0)
      success = vchi_held_msg_release( &msg );
 ...
 *
 ***********************************************************/
int32_t vchi_bulk_queue_receive( const VCHI_SERVICE_HANDLE_T handle,
                                 void * const data_dst,
                                 const uint32_t data_size,
                                 const VCHI_FLAGS_T flags,
 ...
 *
 ***********************************************************/
int32_t vchi_bulk_queue_transmit( const VCHI_SERVICE_HANDLE_T handle,
                                  const void * const data_src,
                                  const uint32_t data_size,
                                  const VCHI_FLAGS_T flags,
                                  void * const bulk_handle )
{
