 ...
 * -------------------------------------------------------------------- */
VCHI_MQUEUE_T *
vchi_mqueue_create( const char *name, int nqueues, int nelements, int element_size )
{
   int32_t success;
   int i;
   unsigned char *ptr;
   ...
   VC_DEBUG( MemAllocTrace, "name:%s nqueues:%d nelements:%d element_size:%d", name, nqueues, nelements, element_size );
   ...
   mq = os_malloc( mq_size, OS_ALIGN_DEFAULT, name );
   os_assert( mq );
 ...
 * -------------------------------------------------------------------- */
void
vchi_mqueue_debug( VCHI_MQUEUE_T *mq, void (*func)( int idx, void *elem ) )
{
   int i;
