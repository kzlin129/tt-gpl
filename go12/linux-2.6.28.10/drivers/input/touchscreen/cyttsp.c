...
static int cyttsp_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_set_abs_params(cyttsp_cap_ts_input_dev, ABS_X, XMIN, XMAX, 0, 0);
	...
	input_set_abs_params(cyttsp_cap_ts_input_dev, ABS_FINGER2_X, XMIN, XMAX, 0, 0);
	...
	if ( drv_data->current_mode == OPERATING_MODE )
	{
		printk(KERN_INFO CYTTSP_DEVNAME ": TS already in operation!\n");
	}
	else if (cyttsp_restart_ts()) {
