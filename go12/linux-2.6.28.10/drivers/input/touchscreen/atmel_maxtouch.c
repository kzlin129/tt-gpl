...
static char fw_ver[16] = "unknown";
static char cfg_ver[32] = "unknown";

module_param(debug, int, 0644);

module_param_string(fw_ver,fw_ver,sizeof(fw_ver), 0444);
module_param_string(cfg_ver,cfg_ver,sizeof(cfg_ver), 0444);

MODULE_PARM_DESC(debug, "Activate debugging output");

static struct workqueue_struct *ts_workq;
...
/* cfg Info descriptor */
#define PANEL_ELK_4INCH 1
#define PANEL_ELK_5INCH 2

struct cfg_info {
        u8   panel;        /* defined as above PANEL_xxx */
	u8   fw_major;     
	u8   fw_minor;     
	u8   fw_build;     
	u8   cfg_major;    
...
static void mxt_input_report(struct mxt_data *data, int single_id)
{
	...
	for (id = 0; id < MXT_MAX_NUM_TOUCHES; id++) {
		...
		if (finger[id].status == MXT_MSGB_T9_RELEASE){
			finger[id].status = 0;
			finger[id].area = 0;
		} else {	
			finger_num++;
...
void process_T9_message(u8 *message, struct mxt_data *mxt)
{
	...
	/* The fingerdata is apparently not in the first reported object */
	report_id = message[0];
	...
	if (!(status & MXT_MSGB_T9_DETECT)) {
		if (status & MXT_MSGB_T9_RELEASE) {
			...
			mxt_input_report(mxt, touch_number);
		}
		return;
	}
	/* Check only AMP detection */
	if (!(status & (MXT_MSGB_T9_MOVE | MXT_MSGB_T9_PRESS)))
		return;
	...
	finger[touch_number].status = status & MXT_MSGB_T9_MOVE ?
				MXT_MSGB_T9_MOVE : MXT_MSGB_T9_PRESS;
	...
	mxt_debug(DEBUG_TRACE, "X=%d, Y=%d, TOUCHSIZE=%d TOUCH NUMBER: %d\n",
		xpos, ypos, touch_size, touch_number);
...
static void mxt_worker(struct work_struct *work)
{
	...
	message_length = mxt->message_size;//always 9 bytes for T9
...
static irqreturn_t mxt_irq_handler(int irq, void *_mxt)
{
	...
	mxt->irq_counter++;
	/* Triggering on a falling edge doesn't seem to work
	 * so we have to disable the interrupt here otherwise
	 * this handler gets called always since we're triggering
	 * on low level now.
	 */
	disable_irq(mxt->irq);
	queue_work(ts_workq, &mxt->dwork);
...
/******************************************************************************/
static int reset_chip(struct mxt_data *mxt)
{
        return mxt_write_byte(mxt->client,
                        MXT_BASE_ADDR(MXT_GEN_COMMANDPROCESSOR_T6, mxt) + MXT_ADR_T6_RESET,
                        1);  /* Any non-Zero value will reset the chip */
}
				  ...
				  u8 *id_block_data)
{
	...
	snprintf(fw_ver, sizeof(fw_ver), "%d%d_%X",
	    mxt->device_info.major,
		mxt->device_info.minor,
		mxt->device_info.build);
...
/*
 * Read User Data SPT_USERDATA_T38 
 *
 */
static int __devinit mxt_read_user_data(struct i2c_client *client,
					   struct mxt_data *mxt)
{
    struct mxt_object *object_table;
    struct cfg_info cfg;
    ...
    error = mxt_read_block(client, MXT_BASE_ADDR(MXT_USER_INFO_T38, mxt), sizeof(cfg), &cfg);
    if (error < 0) {	
			dev_err(&client->dev,
				"MXT_USER_INFO_T38 could not be read\n");
			error = -EIO;
			goto err_read_info;
	}
	
	
    snprintf(cfg_ver, sizeof(cfg_ver), "%d_%d%d_%X_%d%X",
	    cfg.panel,
	    cfg.fw_major,
		cfg.fw_minor,
		cfg.fw_build,
			       ...
			       const struct i2c_device_id *id)
{
	...
	/* Init workqueue */
	ts_workq = create_singlethread_workqueue("touchscreen");
	...
	input_set_abs_params(input, ABS_Y, 0, mxt->max_y_val, 0, 0);
        input_set_abs_params(input, ABS_FINGER2_X, 0, mxt->max_x_val, 0, 0);
        input_set_abs_params(input, ABS_FINGER2_Y, 0, mxt->max_y_val, 0, 0);
	...
	input_set_abs_params(input, ABS_TOOL_WIDTH, 0, MXT_MAX_REPORTED_WIDTH,
			     0, 0);
        input_set_abs_params(input, ABS_FINGERS, 0, 2, 0, 0);
	...
	if (error < 0)
        ...
        error = mxt_read_user_data(client, mxt);
	if (error < 0)
		goto err_read_ot;
...
static int mxt_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	mxt_debug(DEBUG_TRACE, "maXTouch driver SUSPEND\n");
	disable_irq(mxt->irq);
	cancel_work_sync(&mxt->dwork);
	
	/* Leave the input layer in a known state */
	input_report_abs(mxt->input, ABS_FINGERS,  0);
...
static int mxt_resume(struct i2c_client *client)
{
	...
	mxt_debug(DEBUG_TRACE, "maXTouch driver RESUME\n");
