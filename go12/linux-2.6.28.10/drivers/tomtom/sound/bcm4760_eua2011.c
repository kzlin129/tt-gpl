	...
	if (c->tt_volume_dB < VOLUME_MUTE) {
		...
		case TT_SCENARIO_HF_SPEAKER:
			...
			if (c->has_5V_booster == 0) {
				output_gain = (0x01ff - (int)((c->tt_max_speaker_level >> OUTPUT_HF_ATT_NO_5V_BOOSTER_SHIFT) & OUTPUT_MASK));
			} else {
				output_gain = (0x01ff - (int)((c->tt_max_speaker_level >> OUTPUT_HF_ATT_5V_BOOSTER_SHIFT) & OUTPUT_MASK));
			}
			break;
		case TT_SCENARIO_SPEAKER:
			if (c->has_5V_booster == 0) {
				output_gain = ((0x1ff -(int)((c->tt_max_speaker_level >> OUTPUT_ATT_NO_5V_BOOSTER_SHIFT) & OUTPUT_MASK)) - (c->tt_volume_dB << 2));
			} else {
				output_gain = ((0x1ff -(int)((c->tt_max_speaker_level >> OUTPUT_ATT_5V_BOOSTER_SHIFT) & OUTPUT_MASK)) - (c->tt_volume_dB << 2));
			}
...
static int bcm4760_set_speaker_level(struct tt_scenario_context *c)
{
	/* 0xffffffff = use original default levels */
	if (c->tt_speaker_level == 0xffffffff) {
		c->tt_max_speaker_level = ((OUTPUT_ATT_5V_BOOSTER << OUTPUT_ATT_5V_BOOSTER_SHIFT) |
		                          (OUTPUT_ATT_NO_5V_BOOSTER << OUTPUT_ATT_NO_5V_BOOSTER_SHIFT) |
		                          (OUTPUT_HF_ATT_5V_BOOSTER << OUTPUT_HF_ATT_5V_BOOSTER_SHIFT) |
		                          (OUTPUT_HF_ATT_NO_5V_BOOSTER << OUTPUT_HF_ATT_NO_5V_BOOSTER_SHIFT));
	} else
		c->tt_max_speaker_level = c->tt_speaker_level;
...
static struct tt_scenario_ops bcm4760_scenario_ops = 
{
	...
	.set_speaker_level = bcm4760_set_speaker_level,
#ifdef INCLUDE_TEST_KCONTROLS
