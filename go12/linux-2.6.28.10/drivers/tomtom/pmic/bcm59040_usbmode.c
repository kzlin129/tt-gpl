...
//	charger_config->in_maintchrg_mode = 0;
	...
	data = defaults->ntcctrl2;
	if((rc = pmu_bus_seqwrite(pclient, BCM59040_REG_NTCCTRL2, &data, 1)) < 0) {
		printk (KERN_ERR PFX "error writing BCM59040_REG_NTCCTRL2 register.\n");
		return rc;
	}

	/* Lastly, set the temperature hysteresis for the main battery. */
	data = defaults->mbtemphys;
	if((rc = pmu_bus_seqwrite(pclient, BCM59040_REG_MBTEMPHYS, &data, 1)) < 0) {
		printk (KERN_ERR PFX "error writing BCM59040_REG_MBTEMPHYS register.\n");
		return rc;
	}
