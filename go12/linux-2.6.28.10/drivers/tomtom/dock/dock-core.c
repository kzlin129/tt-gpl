...
/* Mute pin interface */
static ssize_t dock_read_mute_pin(struct device *dev, struct device_attribute *attr,
		                                       char *buf)
{
	snprintf (buf, PAGE_SIZE, "%d\n", dock_data.pd->dock_get_mute(dock_data.pd));

	return strlen(buf)+1;
}

static ssize_t dock_write_mute_pin(struct device *dev, struct device_attribute *attr,
		                                        const char *buf, size_t n)
{
	...
	/* sanity check */
	if ( (state == 0) || (state == 1) ){
		dock_data.pd->dock_set_mute(dock_data.pd, state);
		return n;
	}
...
static DEVICE_ATTR(dock_mute_pin, S_IRUGO | S_IWUSR, dock_read_mute_pin, dock_write_mute_pin);
...
{
	DBG(KERN_DEBUG "Configuring dock lines\n");
...
static void dock_set_line_dc_out(struct dock_platform_data *pd, int value)
{
	if (pd->dock_line_set)
		...
		DBG("%s: dock_line_set is NOT set\n", __func__);
	}
}
...
static int dock_get_detid(void)
{
	...
	DBG(KERN_NOTICE PFX " Resetting dock\n");
	...
	if (detid != 0) {
		...
		DBG(KERN_NOTICE PFX " Enable dock power dock\n");
		dock_set_line_dc_out(pd, DOCK_LINE_ASSERT);
		msleep(3); /* Let external signal stabilize */
		detid |= DETID_PWR(dock_get_line_sense0(pd), dock_get_line_sense1(pd));

		/* Deassert dock reset */
		DBG(KERN_NOTICE PFX " Stop resetting dock\n");
		dock_set_line_reset(pd, DOCK_LINE_DEASSERT);
	}
	msleep(3);

	DBG(KERN_INFO PFX "Detected dock with detid [%d]\n", detid);
	return detid;
}
...
static int _dock_state_change_handler(struct dock_type *newdockp)
{
	...
	if (newdockp == &dock_type_invalid) {
		DBG(KERN_NOTICE PFX " Unrecognized dock inserted.\n");
	}

	if (currdockp != newdockp) {
		DBG(KERN_INFO PFX " Dock change detected: %s [%d] => "
		...
		if (currdockp->disconnect && currdockp->disconnect(&dock_data)) {
			DBG(KERN_ERR PFX " Disconnect of current dock [%d] "
				"unsuccessful\n", currdock);
		}

		/* Disable dock power if we don't have a valid dock */
		if (newdockp == &dock_type_none || newdockp == &dock_type_invalid) {
			DBG(KERN_INFO PFX " Disabling dock power and activate reset\n");
...
static struct dock_type *_dock_lookup(int detid)
{
	...
	list_for_each(entry, &dock_type_table) {
		...
		if (dtp->detection_id == detid) {
			...
			if (t > prio) {
				DBG(KERN_NOTICE PFX "Priority is %d > %d\n", t, prio);
				prio		= t;
				best_type	= dtp;
			}
		}
	...
	DBG(KERN_NOTICE PFX "Found new dock: %s\n", best_type->dock_name);
...
static int _dock_force_state(struct dock_type *dtp)
{
	...
	DBG(KERN_INFO PFX " Forcing dock state to %s [%d] ...\n",
...
int setup_main_interface(struct platform_device *pdev)
{
	...
	if (device_create_file(&pdev->dev, &dev_attr_dock_mute_pin)) {
		dev_err(&pdev->dev,
			"Could not create dock_mute_pin sysfs attribute\n");
	}
	return 0;
}
...
void destroy_main_interface(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_dock_mute_pin);
}
...
int dock_get_i2c_irq(void)
{
	...
	if (!irqres) {
		DBG(KERN_ERR PFX  " Missing IRQ resources!\n");
		return -EINVAL;
	}
	return irqres->start;
}
...
void dock_type_register(struct dock_type *dtp)
{
	...
	DBG(KERN_INFO PFX "Registered new dock %s\n", dtp->dock_name);
...
void dock_type_unregister(struct dock_type *dtp)
{
	...
	DBG(KERN_INFO PFX "Unregistered dock %s\n", dtp->dock_name);
}
...
static ssize_t store_forced_dock(
	...
	DBG(KERN_INFO PFX " Forcing dock detection ID to [%#x]\n", newdock_detid);
...
static int __init dock_init_module(void)
{
	...
	if (ret) {
		DBG(KERN_ERR PFX " Unable to register driver (%d)\n", ret);
		return ret;
	}
