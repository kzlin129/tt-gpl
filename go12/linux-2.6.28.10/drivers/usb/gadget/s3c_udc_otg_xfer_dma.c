...
#ifdef CONFIG_USB_S3C_OTGD_NO_XVBUS
#define is_otg_core_reset(x)	(1)
#else
static inline int is_otg_core_reset(u32 usb_status)
{
	return ((usb_status & 0xc0000) == (0x3 << 18));
}
#endif
...
static irqreturn_t s3c_udc_irq(int irq, void *_dev)
{
	...
	if (intr_status & INT_RESET) {
		...
		if(is_otg_core_reset(usb_status)) {
			if(reset_available) {
