...
static struct s3c_otg_platform_data *s3c_otg_phy;
...
static void udc_disable(struct s3c_udc *dev)
{
	...
	if (s3c_otg_phy && s3c_otg_phy->otg_phy_off)
		s3c_otg_phy->otg_phy_off();
}
...
static int udc_enable(struct s3c_udc *dev)
{
	...
	if (s3c_otg_phy && s3c_otg_phy->otg_phy_init)
		s3c_otg_phy->otg_phy_init();
...
static void reset_usbd(void)
{
	...
	utemp = readl(S3C_UDC_OTG_DIEPCTL(0));

	if (utemp & DEPCTL_EPENA) {
		writel(utemp|DEPCTL_EPDIS, S3C_UDC_OTG_DIEPCTL(0));
		while(readl(S3C_UDC_OTG_DIEPCTL(0)) & DEPCTL_EPENA);
	}

	utemp = readl(S3C_UDC_OTG_DIEPCTL(0));
	if (utemp & DEPCTL_NAKSTS)
		writel(utemp|DEPCTL_CNAK, S3C_UDC_OTG_DIEPCTL(0));

	for (i = 1; i < S3C_MAX_ENDPOINTS; i++) {
		utemp = readl(S3C_UDC_OTG_DIEPCTL(i));

		writel(0x0, S3C_UDC_OTG_DIEPCTL(i));
		writel(0x0, S3C_UDC_OTG_DOEPCTL(i));

		if (utemp & DEPCTL_EPENA) {
			writel(utemp | DEPCTL_SNAK | DEPCTL_EPDIS, S3C_UDC_OTG_DIEPCTL(i));
			while(readl(S3C_UDC_OTG_DIEPCTL(i)) & DEPCTL_EPENA);
		}
	}
...
static int s3c_udc_probe(struct platform_device *pdev)
{
	...
	s3c_otg_phy = pdev->dev.platform_data;
	...
	retval =
	    request_irq(IRQ_OTG, s3c_udc_irq, IRQF_SHARED, driver_name, dev);
