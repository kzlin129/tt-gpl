...
{
	writel(pre_margin, IO_ADDRESS(WDT_R_WDOGLOAD_MEMADDR));
}
...
static int watchdog_open(struct inode *inode, struct file *file)
{
	...
	rc = request_irq(bcm4760wdt_irq, wdog_interrupt, 0, "wdog", NULL );
	if( rc )
	{
...
#ifdef ALLOW_ISR_TO_CLEAR_INT_AND_DISALLOW_RESET
	free_irq(bcm4760wdt_irq, NULL );
#endif

	allow_close = CLOSE_STATE_CLOSED;
	up(&open_lock);
...
static char banner[] __initdata =
	KERN_INFO PFX "BCM476X Watchdog driver\n";
...
static int bcm4760wdt_probe(struct platform_device *pdev)
{
	int ret;
	struct resource *res;
	...
	/*
	 * Cycle the WDT 32K clock reset
	 */
	clear_bit(CMU_F_CMU_WDT_32KRESETN_R,
			  (unsigned long*)IO_ADDRESS(CMU_R_BLOCK_RESET2_MEMADDR));
	msleep(1);
	set_bit(CMU_F_CMU_WDT_32KRESETN_R,
			(unsigned long*)IO_ADDRESS(CMU_R_BLOCK_RESET2_MEMADDR));
	msleep(1);
...
static int bcm4760wdt_remove(struct platform_device *dev)
{
...
static int bcm4760wdt_suspend(struct platform_device *dev, pm_message_t state)
{
	if (allow_close != CLOSE_STATE_CLOSED) {
		watchdog_ping();
	}
...
static int bcm4760wdt_resume(struct platform_device *dev)
{
	if (allow_close != CLOSE_STATE_CLOSED) {
		watchdog_start();
		watchdog_ping();
...
static void bcm4760wdt_shutdown(struct platform_device *dev)
{
}

static struct platform_driver bcm4760wdt_driver = {
	.probe		= bcm4760wdt_probe,
	.remove		= bcm4760wdt_remove,
	.shutdown	= bcm4760wdt_shutdown,
	.suspend	= bcm4760wdt_suspend,
	.resume		= bcm4760wdt_resume,
	.driver		= {
		.owner  = THIS_MODULE,
		.name   = "bcm4760-wdt",
	},
...
{
	printk(banner);
	return platform_driver_register(&bcm4760wdt_driver);
}

static void __exit watchdog_exit(void)
{
	platform_driver_unregister(&bcm4760wdt_driver);
}
