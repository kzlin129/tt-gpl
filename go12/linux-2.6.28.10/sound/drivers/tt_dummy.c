/*
 *  Dummy soundcard
 *  Copyright (c) TomTom International BV <martin.jackson@tomtom.com>
...
MODULE_DESCRIPTION("TT Dummy soundcard with Nashville (/dev/null)");
MODULE_LICENSE("GPL");
...
static int __devinit snd_dummy_probe(struct platform_device *devptr)
{
	...
	for (idx = 0; idx < MAX_PCM_DEVICES && idx < pcm_devs[dev]; idx++) {
		...
		/* add TT specific controls */
		err = tomtom_add_nashville_controls(dummy->card, dummy);
		if (err < 0)
			goto __nodev;
	}
