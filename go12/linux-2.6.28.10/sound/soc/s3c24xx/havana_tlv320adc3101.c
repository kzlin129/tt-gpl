/*
 * havana_tlv320adc3101.c  --  SoC audio for S5P6440 Havana with TLV320ADC3101
 *
...
static int havana_socmst_hw_params(struct snd_pcm_substream *substream,
...
static const struct soc_enum havana_enum[] = {
...
static int havana_get_linein(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = havana_linein_func;

	return 0;
}

static int havana_set_linein(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (havana_linein_func == ucontrol->value.integer.value[0])
		return 0;

	havana_linein_func = ucontrol->value.integer.value[0];
	havana_ext_control(codec);

	return 1;
}

static int havana_get_micin(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = havana_micin_func;

	return 0;
}

static int havana_set_micin(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (havana_micin_func == ucontrol->value.integer.value[0])
		return 0;

	havana_micin_func = ucontrol->value.integer.value[0];
	havana_ext_control(codec);

	return 1;
}

static int havana_get_digimicin(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = havana_digimicin_func;

	return 0;
}

static int havana_set_digimicin(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (havana_digimicin_func == ucontrol->value.integer.value[0])
		return 0;

	havana_digimicin_func = ucontrol->value.integer.value[0];
	havana_ext_control(codec);

	return 1;
}

static const struct snd_kcontrol_new adc3101_havana_controls[] = {
/* Line In Jack control */
	SOC_ENUM_EXT("Line In Jack", havana_enum[0], 
			havana_get_linein, havana_set_linein),
/* Mic In Jack control */
	SOC_ENUM_EXT("MIC In Jack", havana_enum[1], 
			havana_get_micin, havana_set_micin),
/* Digital Mic In control */
	SOC_ENUM_EXT("Digital MIC In", havana_enum[2], 
			havana_get_digimicin, havana_set_digimicin),
};

static int havana_tlv320adc3101_init(struct snd_soc_codec *codec)
{
	int i, err;

	/* Add adc3101 specific controls */
	for (i = 0; i < ARRAY_SIZE(adc3101_havana_controls); i++) {
		err = snd_ctl_add(codec->card, 
				snd_soc_cnew(&adc3101_havana_controls[i], codec, NULL));
	if (err < 0)
		return err;
	}
...
static int havana_startup(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
static void havana_shutdown(struct snd_pcm_substream *substream)
{
}
...
static struct snd_soc_ops havana_ops = {
	.hw_params = havana_socmst_hw_params,
	.startup = havana_startup,
	.shutdown = havana_shutdown,
};
...
static struct snd_soc_dai_link havana_dai[] = {
{
	...
	.init = havana_tlv320adc3101_init,
	.ops = &havana_ops,
},
};
...
static int __init havana_audio_init(void)
{
	...
	havana_snd_device = platform_device_alloc("soc-audio", 0);
...
MODULE_DESCRIPTION("ALSA SoC Havana TLV320ADC3101");
MODULE_LICENSE("GPL");
